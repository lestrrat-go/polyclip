...
- **General non-convex crossing-splice — FIXED (2026-05-20); `TestUnionAllManyOverlapping` un-skipped and passing.** Two coupled premature-mutation bugs broke `getPrevHotEdge`'s search for the *enclosing* hot edge, which is what sets a crossing-spawned ring's front/back orientation:
  ...
  **Shared-vertex EXIT via a horizontal — LANDED (2026-05-21, this session).** A through-vertex hand-off that `handoffMaxThroughVertex` could not fire because the through-edge sat on a *horizontal*. Repro `A=(5,1),(6,5),(5,5),(1,7) ; B=(1,1),(7,0),(7,3),(6,5)` (shared vertex (6,5)): Union 16 vs 22.15, Difference 0 vs 6.17 (Intersect/Xor already correct). B reaches its local MAX at (6,5); A passes THROUGH (6,5), turning there onto its top horizontal (6,5)→(5,5) before continuing to its apex (1,7). A's local min (5,1) is interior to B, so A is cold below (6,5) and must become the union boundary above it — exactly the hand-off `handoffMaxThroughVertex` performs at B's max. **Root cause:** the hand-off's apex-column test was `XAtY(c.Seg, maxPt.Y) == maxPt.X`, and at the confluence A's cursor sat on its horizontal (6,5)→(5,5); `XAtY` returns a horizontal's `Bot.X` (the x=5 end) rather than the through-vertex end (x=6), so A's through-edge failed the test and was rejected. A never went hot and its whole upper region dropped. **Fix:** new `throughVertexOnColumn(seg, maxPt)` — for a sloped segment it is the old `XAtY == maxPt.X`; for a horizontal it tests that `maxPt` lies on the horizontal's x-span at `maxPt.Y` (the through-vertex is an *endpoint* of the horizontal, not its `Bot`). One-line semantics, no change to the hand-off's other guards (cold-only, continues-above, not-maxima-partner). Measured on a throwaway [0,8] simple-CCW-quad MC differential (5000 pairs, 120k samples, tol 0.6, per-pair before/after via a `ZZHorizHandoff` toggle), two seeds: **4 pairs fixed, 0 regressed each**; ON-residual U=2 I=1 D=2–3 X=2–5 (pre-existing other classes). Regression guard `TestSharedVertexExitViaHorizontal`. Files: clip/sweep.go (one helper + the guard call), boolean_test.go.
  ...
  **Track C — fuzz-surfaced residual characterized: the trigger is a shared vertex / coincident edge where both sources' bounds meet, NOT coincident horizontals per se (2026-05-22, this session).** CI fuzz (`FuzzUnion/Intersect/Difference/Xor`, run 26281095840) failed all four ops with area-bound violations. Investigation:
  - **Pure diff-source coincident-horizontal *overlap* with no shared vertex is correct for all four ops.** Repro `A=(0,0),(0,5),(3,5),(-1,6) ; B=(1,5),(-1,5),(-2,0),(2,5)` (a's top edge (0,5)→(3,5) and b's top both on y=5, overlapping x∈[0,2], b's flanks sloped so no vertex/edge is shared) — U/I/D/X all within MC tolerance.
  - **Adding a shared vertex breaks it.** `A=(0,0),(0,5),(3,5),(-1,6) ; B=(2,5),(1,5),(1,1),(3,5)` (shares vertex (3,5) at the coincident horizontal) → Union 3.5 < |A|=4 (under-counts). The fuzz inputs hit this *indirectly*: `SplitOverlaps`/`SplitTJunctions` convert a partial coincident-horizontal overlap into shared vertices at the overlap endpoints, so a coincident-horizontal fuzz input is really a shared-vertex input after preprocess. Same root class as the single-shared-vertex residual above — not a separate horizontal bug.
  - **Catastrophic sub-case: a fully-coincident diff-source NON-horizontal (e.g. vertical) edge at a shared local-min confluence.** Repro `A=(0,0),(0,5),(3,5),(-1,6) ; B=(1,5),(0,5),(0,0),(2,5)` (A and B share the whole edge (0,0)–(0,5); A's local-min (0,0) coincides with B's): Union **0**, Intersect **0**, Difference 5 (= |B|), Xor 9 (= |A|+|B|) — the engine treats the pair as entirely non-interacting. Trace (`SweepResult.Trace`) shows every classified edge stuck at `WindSelf=0`/non-contributing: two exactly-coincident diff-source bounds occupy the same AEL column for their whole span, and `Classify`'s same-source-predecessor scan plus the incremental winding update cannot resolve a pile-up of exactly-overlapping edges. This is the "diff-src coincident-edge pile-up" §11.7/§7 (line ~288) flags as not reliably handled; it needs a stable AEL tie-break for exactly-coincident edges (Clipper2 keeps both edges with their own `wind_dx` and a deterministic order) before the winding can register the overlap. **Not yet fixed** — deliberately not band-aided in the dispatch given the documented coin-flip history below. Worktree `fix-coincident-horizontal`. Validation method for any fix: the three repros above + full clip/polyclip unit suite green + the small-coord ([0,6]) MC-oracle differential staying at 0 wrong (current baseline: 0/4487 interacting pairs — the class is too narrow for random quads to hit, so the repros are the primary guard).

  **Mechanism pinned + dispatch-skip fix attempts are COIN-FLIPS (2026-05-22, this session).** Instrumented the bound model (throwaway `clip/zz_dbg.go` trace hook) on the shared-vtx repro. The premature drop happens in `dispatchIntersect`: `SplitOverlaps`/`SplitTJunctions` fragment the coincident y=5 horizontal into pieces, and `IntersectEdges` fires at every piece boundary. At a MID-plateau piece where BOTH bounds carry the coincident horizontal on collinearly to a shared apex ((3,5) in the repro), the existing coincident-skip is blocked (`continuesCollinearHorizontal` is true for both AND neither is `IsBoundLast`), so dispatch falls to the both-hot branch → `AddLocalMaxPoly` → **closes BOTH rings at (1,5)**, dropping everything beyond (Union 3.5 vs truth 8). Three local-signal gates were tried to extend the skip to this "both continue collinearly" case, each a coin-flip on the six original fuzz repros (validated vs MC oracle):
  1. skip when `c1 && c2` (both continue) — fixes shared-vtx + orig-Diff(D), **regresses** horiz-only A (a genuine *overlap*, I=1.65: its coincident piece at (0,5) is one-hot and needs the one-hot transfer, not a skip; D 2.35→0.43).
  2. gate (1) on `e1Hot && e2Hot` — fixes horiz-only A, **regresses** orig-Diff(D 175→320) + orig-Union (both are touch cases whose decisive coincident crossing is one-hot, so they need the skip the hotness gate now denies).
  3. gate (1) on `WindOther == 0` (the touch signal: edge on the other source's *outer* boundary, seen as `wo=0` at the orig-Diff (15,5) and shared-vtx (1,5) crossings vs `wo=1` at horiz-only A's (0,5)) — **regresses** shared-vtx Union, because a *different* coincident crossing in the same op has `wo≠0` and the uniform gate misfires there.
  **Conclusion (firsthand, matching the deadend log):** a single local predicate on the dispatch-skip cannot separate touch (skip) from overlap (one-hot handoff) — DIFFERENT coincident crossings WITHIN ONE op need OPPOSITE decisions, so any uniform gate fixes some and breaks others. The skip itself is a workaround for polyclip not classifying coincident diff-source edges through the standard `wind_cnt2` model (Clipper2 has NO such skip — both coincident edges live in the AEL with their own `wind_dx` and the ordinary contributing test resolves each piece). The correct fix is structural: give exactly-coincident diff-source edges a deterministic AEL order and let `Classify`/`isContributing` decide each piece per `wind_cnt2`, then delete the `dispatchIntersect` coincident-skip (and its `continuesCollinearHorizontal`/`respawnHandoffAtOverlap` helpers) entirely. Not a one-session change. All speculative gates reverted.

  **LANDED — fixed at the INPUT layer instead of the dispatch (2026-05-22, this session).** The coin-flip above is in `dispatchIntersect`; the actual trigger is upstream and removable. The shared-vtx repro's B carries a REDUNDANT COLLINEAR vertex `(2,5)` on its top edge (`(3,5),(2,5),(1,5)` are collinear), and `SplitTJunctions` then splits A's coincident horizontal at that x, manufacturing the mid-plateau coincident piece whose crossing prematurely closed both rings. The vertex is geometrically redundant — removing it leaves the polygon identical — but the Vatti bound model treats its shared endpoint as a spurious turn. **Fix (`boolean.go`): `simplifyCollinearRing`, called from `appendRing` on the snapped ring before segment emission**, drops every vertex with `fixed.Orient2D(prev,cur,next)==0` (consecutive- and wrap-duplicates removed first so a zero-length neighbour can't be misread as collinear and delete a real corner). An exact geometric no-op for any simple polygon, so general position is byte-identical (zero regression by construction). Fixes the shared-vtx repro AND the `FuzzUnion/a61d…` + `FuzzDifference` corpus seeds (Union 3.5/369.5 → exact, Difference 320 → 175 ≈ MC); the `FuzzIntersect`/`FuzzXor` seeds also stop violating their area bounds though residual MC-error from the separate cross-source-collinearity (Int) and vtx-on-edge (Xor) classes remains. Validated: full clip+polyclip+fixed suite green, `go vet` clean, gofmt clean; degenerate [0,6] MC differential 0 wrong / 3352 interacting (= baseline, no regression). Guards `TestSharedVertexCollinearHorizontalSimplified` (exact integer truth: A,B touch so U=8 I=0 D=4 X=8) + `TestSimplifyCollinearRing` (unit). The `dispatchIntersect` coincident-skip is untouched and still load-bearing for genuine coincident-overlap (no redundant vertex) cases. **Remaining residual: `FuzzIntersect/ff9…` (cross-source collinearity — A's edge on y=x meets B's vertical x=3) and `FuzzXor/28f…` (collinear vertex removed but a vtx-on-edge of the plateau corner on B's vertical remains) — different classes, next increments.**
  ...
  **d50048a same-side recovery FIXED in place — LANDED (2026-05-22, this session). The "must port SetOwner/Split/SwapFrontBackSides upstream" framing above is OVERTURNED for the same-side-at-maximum collision: the inverted orientation does NOT have to be prevented upstream — it can be DETECTED and corrected at the maximum, where the geometry is unambiguous.** The d50048a workaround reversed a *fixed* edge (`e2`) whenever two different rings met same-side at a local max. That is a coin flip: the genuinely-inverted ring can be e1's *or* e2's, and reversing the already-correct ring is what tangled the merged chain. **Fix (`AddLocalMaxPoly`, clip/output.go):** at the maximum the AEL order is decisive — in polyclip's mirror convention the RIGHT (higher AEL index) bound is its ring's `FrontEdge` and the LEFT bound is its `BackEdge`. Re-derive left/right from `ael.IndexOf`, then reverse whichever ring disagrees (right-not-front, else left-is-front) with the head-shift `SwapFrontBackSides` requires; if either edge has already left the AEL (`IndexOf<0`) fall back to the legacy `e2` reversal. **Repro** (the vertex-on-edge same-side tangle): `Xor A=(2,2),(7,2),(5,5),(3,5) ; B=(0,0),(7,7),(3,4),(2,6)` — A's vertices (2,2),(5,5) both lie on B's diagonal (0,0)-(7,7); SplitTJunctions splits it at both, and the cross-source crossings reorder ownership so B's two bounds reach its apex (7,7) BOTH back-side on different rings (r0,r1). r1's front/back was inverted by an upstream `SwapOutrecs`. Pre-fix the workaround reversed r0 (already correct) → self-touching ring whose sub-loops cancel → Xor 13.2; post-fix it reverses r1 → Xor 14.53 (MC truth 14.526; U/I/D were already correct and unchanged). **Validated against a Monte-Carlo area oracle, NOT Clipper2** — Clipper2 is itself WRONG on this tiny-integer degenerate input (built + ran: its Xor = 13.5, Union 17.0, Intersect 2.0, Difference 7.5, all off the MC truth; polyclip already beat it on U/I/D and now matches on Xor too). Measured per-pair before/after (toggle on the recovery path) over the [0,8] simple-CCW-quad MC differential, 4 seeds × 400 pairs (~930 interacting): **8 gross fixes (Union/Difference/Xor), ZERO regressions** (wrongOld 26 → wrongNew 18; the remaining 18 are other degenerate classes untouched by this branch). Guard `TestXorVertexOnEdgeSameSideTangle`. Files: clip/output.go (the recovery rewrite). NOTE: this does not delete the workaround — it makes it correct; the broader OutRec-orientation port remains the eventual cleanup, but is no longer load-bearing for this class.
...
**Out of scope for this increment / remaining caveats.** Degenerate positions (shared vertices, vertex-on-edge T-junctions, collinear overlaps that survive preprocess) are a separate hardening task; the differential harness should be re-run with a *generic-position* filter to confirm the crossing-count failure rates collapse, then separately with degeneracies allowed to size the remaining work.
  ...
  **Handoff qualification — cursor-timing bug FIXED (2026-05-21, later session): condition (c) is the bound's apex, not `!IsBoundLast`.** The `handoffMaxThroughVertex` (c) test above used `!c.IsBoundLast()` as a proxy for "the through edge continues above maxPt". That proxy is wrong: it depends on where the through-bound's *cursor* sits at the instant `ae`'s maximum closes, which is timing-dependent at a confluence. Two variants of the SAME through-vertex configuration arise:
  - The (10,4) repro: when A closes, B's cursor is still on the segment ENDING at the shared vertex (`(7,1)→(10,4)`, Top == maxPt) — that segment is *not* B's last, so `!IsBoundLast` = true and the handoff fired (correct).
  - The (9,8) repro `Union((0,8),(5,3),(3,6),(9,8) ; (2,9),(3,7),(9,8),(5,11))`: when A closes, B's cursor has already advanced onto the segment LEAVING the shared vertex (`(9,8)→(5,11)`, Bot == maxPt) — and that segment *is* B's last (B's apex (5,11) is one segment up). So `!IsBoundLast` = false, the handoff wrongly skipped, B's upper triangle dropped, and the union collapsed to 14.75 (truth ~23.23). A current-segment `Top.Y > maxPt.Y` test has the mirror-image failure (right for (9,8), wrong for (10,4)).

  **Fix:** decide (c) from the bound's *ultimate apex* — `boundContinuesAbove(c, maxPt) == c.Bound.Last().Top.Y > maxPt.Y` — which is independent of the cursor's segment. Both timings now qualify identically (a genuine maximum, whose bound tops out *at* maxPt, still fails the test and is left to the partner / confluence logic). The COLD guard (d) is unchanged, so hot-through confluences are still deferred. Guard: `TestUnionThroughVertexBoundLastSegment` (the (9,8) repro). **Effect (same 4000-pair differential, before→after):** Union 7→5, Difference 10→7, Intersect 0, Xor 3 (unchanged — still the hot-through class). Per-pair before/after diff: **5 pairs fixed (2 Union, 3 Difference), zero regressions.** Full suite green. The remaining residual is unchanged in character: Xor's 3 gross failures (and the Union/Difference cases that still fire nothing) are the **hot-through-edge shared-apex confluence** — tracing the (9,8) Xor case shows the sweep emits a single tangled self-touching ring (the shared vertex visited 2–3×, with two correct sub-loops cross-joined at reversed orientation via the d50048a workaround).

  **Hot-through shared-apex confluence — FIXED (2026-05-22): defer the plateau maximum until its horizontal partner arrives (`plateauPartnerPending`).** The (9,8) Xor repro `Xor((0,8),(5,3),(3,6),(9,8) ; (2,9),(3,7),(9,8),(5,11))` returned 0.25 vs truth ~20.0. Root cause traced (clip-package descale trace) and confirmed against real Clipper2 (built scaled ×1000 to avoid integer-snap noise; Clipper2 yields **two disjoint CCW rings** — (A\B)=8.25 + (B\A)=11.75 = 20.0 — that touch at (9,8) and the crossing (2.5,8) but do NOT merge). The bug is a **bound-construction + event-order divergence at a local-max plateau**, NOT an orientation-parity bug:
  - polyclip's `buildBoundsAt` trims a top horizontal onto the *left/backward* bound, so BOTH of A's bounds end at the plateau's right end (9,8) and A's right bound `(3,6)→(9,8)` fires a standalone `EventTop` maximum there. Clipper2 instead keeps the top horizontal on the *forward* bound (traversed leftward to (0,8)); A's right-bound vertex (9,8) is therefore **intermediate** (`IsMaxima` false), so `DoMaxima` returns early (`GetMaximaPair` is null — "eMaxPair is horizontal") and A's right bound STAYS hot in the AEL. The whole plateau maximum is closed later inside `DoHorizontal` when the traversing horizontal reaches the partner edge (`e->vertex_top == vertex_max`, engine.cpp:2578).
  - In polyclip, A's right bound closes FIRST via `closeBound`'s Case A deferred handoff (removed from the AEL, OutRec coupling left intact for the horizontal's Case B). Then `flushPendingHoriz` traverses A's top horizontal, which crosses B's left bound at (2.5,8) — a both-hot cross-source Xor crossing → interleave `SwapOutrecs`, moving the horizontal into B's OutRec. So A's plateau ring never closes cleanly; B's two bounds end up **same-side** (both back edges of different OutRecs) at B's apex (5,11), tripping the d50048a same-side `AddLocalMaxPoly` workaround, which reverses+joins them into one self-touching ring (areas cancel → 0.25).
  - **Fix** (`sweep.closeBound` + new `plateauPartnerPending`): when a non-horizontal bound reaches a maximum whose maxima-partner is nil but whose coupled OutRec partner is a **pending horizontal heading to maxPt** (a plateau whose other end arrives via the horizontal pass), DEFER — leave `ae` hot in the AEL and return. The horizontal's own `closeBound` then finds `ae` via `maximaPartner` and closes the pair, and `resolveBetweenMaxima` crosses any between-edges (here B's hot through-bound at the apex column) FIRST — splitting B's right bound's ring ownership correctly so the two Xor rings stay separate. This emulates Clipper2's "intermediate vertex stays hot, horizontal closes the max" behavior without restructuring bound construction.
  - **Effect:** the (9,8) Xor repro 0.25 → 20.00 (exact, 3 rings: the two disjoint outers + dedup). Per-pair before/after diff over 2500 random single-shared-vertex CCW-quad pairs [0,12] (identical MC truth, plateau-fix toggled off vs on): **1 gross failure fixed (random pair Xor 26.5→48.0), 0 regressions**; two other changed pairs both moved closer to truth. Xor `AddLocalMaxPoly` same-side workaround firings dropped 13→4 over the 2500 pairs (the 9 de-fired pairs kept identical areas — the workaround had been a no-op band-aid for them). Full suite green; guard `TestXorHotThroughSharedApexConfluence`. The d50048a workaround still fires for the **remaining ~4 Xor confluence sub-cases** (e.g. where the partner is NOT reached via a horizontal, or a 4-bound co-maximum); those are the next increment. Method: clip-package descale trace hook (`zz_dbg.go`, deleted) + a throwaway Go-only MC differential with a `DbgDisablePlateau` toggle for the per-pair before/after diff + a `g++` Clipper2 ×1000 oracle, all rebuilt and deleted this session.

  **"Delete the d50048a workaround" increment — REFUTED & RE-SCOPED (2026-05-21, this session).** The queued plan ("drive the remaining ~4 Xor same-side firings to 0, then delete the d50048a workaround") rested on the stale premise that the workaround is a no-op band-aid (true on the older/smaller 2500-pair differential). A broader throwaway differential — **30000 single-shared-vertex CCW-quad pairs [0,12], 5 seeds, vs an MC oracle, with a `DbgSameSideFires` probe AND a `DbgDisableSameSide` toggle** that swaps the recovery for `return nil` to do a per-pair on-vs-off diff — overturns it decisively. Two harness variants were run (each ~175s):

  | harness | wrong | firedPairs | firedWrong | diverged(on≠off) |
  |---------|-------|-----------|-----------|-------------------|
  | unrestricted single-shared-vertex | 28 (0.093%) | 53 | 11 | 50 |
  | **clean** single-shared-vertex (no vertex-on-edge / collinear overlap) | 7 (0.023%) | 35 | 7 | 33 |

  Findings:
  1. **The workaround is LOAD-BEARING, not a no-op.** In the clean class, 28 of 35 firing pairs *recover the correct area* via the reverse+join; disabling it (`return nil`) regresses them grossly (e.g. `on=48.939 off=8.667 truth=48.804`). Deleting it now would BADLY regress. The 2026-05-20 "replacing with `return nil` leaves wrong-rates unchanged" note was measured on a narrower differential and is stale.
  2. **The clean residual is tiny (0.023%) and entirely the coin-flip class** (firedWrong == wrong == 7). Of those 7: 3 would be fixed by `return nil` (rings that should stay SEPARATE but the workaround wrongly reverse+joins), 2 hit the same-ring bail (`e1.Outrec==e2.Outrec`, already mis-merged upstream), 2 are wrong either way.
  3. **There is NO local discriminator at the same-side firing.** Traced a should-merge case (`Xor((9,5),(2,5),(6,1),(10,4) ; (9,5),(3,11),(0,2),(5,8))`, reverse+join correct) against a should-NOT-merge case (`Xor((2,1),(9,11),(0,11),(2,6) ; (9,11),(6,8),(2,12),(5,0))`, `return nil` correct): at the firing both present **identically** — `src=1, e1 windDx=-1 windOther=0 FRONT, e2 windDx=+1 windOther=1 FRONT, e2 ring empty`. The two topologies differ only globally (the should-not-merge case fires a *second* same-side max across sources). So the workaround cannot be made selective by any local test; deciding merge-vs-separate requires a correct OutRec front/back, i.e. **the structural OutRec ownership/orientation port** (maintain front/back through the cross-source crossing-respawn — Clipper2 `SetOwner`/`Split`-style). This re-confirms the two prior DEADEND findings (winding-derived parity; AEL-ordered maxima). It is a multi-session structural increment and was deliberately NOT attempted half-way under a stop-hook (see [[iterative-pacing]]).

  **The dominant gross-error class is NOT the workaround — it is shared collinear HORIZONTAL edges.** In the unrestricted run, 17 of the 28 wrong pairs have `fired=0` (the workaround is never reached). All 8 sampled `fired=0` repros become **correct when the shared vertex is perturbed off-grid** — so they are genuinely degeneracy-caused, not the general-position non-convex residual. Dumping the raw sweep rings (internal `assembleResult`-input dump) for the worst, `Xor((9,4),(0,4),(1,0),(1,3) ; (9,4),(4,11),(2,5),(8,4))` (got 10.0 vs truth 28.6), shows the inputs **share a collinear horizontal edge (8,4)-(9,4)** (A below, B above, interiors opposite), and A's bottom edge `(0,4)-(9,4)` is itself horizontal (split at the T-junction (8,4)). The sweep emits malformed rings (a tangled A/B mix + a negative-area fragment); **Difference (6.0) and Intersect (0.0) are CORRECT, but Union (6.5) and Xor (10.0) drop B's entire above-edge region.** The simplest axis-aligned shared-edge case (two adjacent squares sharing `x=2`) is handled correctly (Union/Xor = 8.0, two clean rings) — so the bug needs the **horizontal collinear overlap + T-junction** combination, in the bound-construction / `doHorizontal` path, not shared edges per se. This is a higher-value real-world case (adjacent polygons sharing a horizontal boundary) and is the **recommended next target** over the coin-flip class.

  **Net for this session:** no source change landed (the queued increment is a confirmed deadend; the workaround stays). Deliverable is this re-scoping. All throwaway scaffolding (`clip/zz_dbg.go`, `zz_xor_diff_test.go`, `zz_perturb_test.go`, `zz_trace_test.go`, `zz_rings_internal_test.go`, and the probe/toggle lines in `clip/output.go`'s same-side branch) was deleted before commit.

  **Shared collinear HORIZONTAL edges — DIAGNOSED as TWO coupled sub-bugs; a partial fix is net-NEGATIVE; full fix is a structural multi-session increment (2026-05-21, later session).** Traced the recommended-next-target class with a clip-package descale trace. **Minimal repro (H4): a triangle sitting on a wider triangle, sharing a partial horizontal edge.** `A=(0,2),(4,2),(3,0)` (top edge horizontal y=2, apex DOWN at (3,0)) ∪ `B=(2,2),(4,2),(3,4)` (bottom edge horizontal y=2 ⊂ A's top, apex UP at (3,4)); shared collinear horizontal (2,2)-(4,2), interiors opposite (A below, B above), T-junction at (2,2). Union got 4.0 (= just A's triangle, B dropped) vs truth ~6.0; Xor got 6.0 (A=4 + B emitted as a separate REVERSED 2-area sliver) vs ~6.0-by-luck-magnitude-wrong-sign. `Difference` and `Intersect` are correct (B's region isn't in their output). The original worst repro (`(9,4),(0,4),(1,0),(1,3) ; (9,4),(4,11),(2,5),(8,4)`) is the same structure at larger scale.

  Two distinct, coupled mechanisms (BOTH must be fixed; fixing only the first regresses):
  1. **Classification — B's bounds misclassified as interior.** A's top horizontal is A's local-MAX *plateau*. When B's local minimum sits in the MIDDLE of that plateau (the T-junction case), A's plateau cannot fully close before B's min is processed: A's right bound reaches the plateau's right end while A's left bound (which `buildBoundsAt` trims the top horizontal onto) is still mid-traversal, so A's terminal horizontal lingers in the AEL when B's min calls `Classify`. `Classify`'s `WindOther` left-walk then counts A's horizontal (`WindDx=+1`) as if A continues above y=2 → B gets `WindOther=1` → non-contributing for Union → B's ring is never created and B is dropped. (Contrast H1, two vertically-stacked squares sharing the FULL edge: there A's max closes as a unit — `AddLocalMaxPoly` with its own top-horizontal partner — *before* B's min, leaving an empty AEL, so B classifies with `WindOther=0`. H1 works; the partial overlap + T-junction is what breaks.) A candidate fix — in `Classify`, skip an other-source edge whose bound apex Y equals the scanline Y (it terminates here, does not extend above the local min) — DOES make B contributing, BUT see (2).
  2. **Horizontal traversal — collinear-overlap mis-cross.** Once B contributes, A's max-plateau horizontal traverses the collinear-overlap span [2,4] (which is COINCIDENT with B's own min-plateau horizontal — both are first-class AEL edges per §12.6.1) and `doHorizontal` spuriously "crosses" B's left bound at the shared NEAR vertex (2,2) where both horizontals begin. `IntersectEdges` reads the post-cross winding as both-non-contributing and fires `AddLocalMaxPoly`, MERGING A's and B's rings into one self-touching ring that cancels B's area. Geometrically B's left bound only TOUCHES the horizontal at (2,2) (it goes up-and-away), so there is no crossing — the boundary should hand off from A's top onto B's left bound. This is the collinear-horizontal overlap cancellation/handoff that Clipper2 handles with its dedicated horizontal-join subsystem (`horz_seg_list_`, `ConvertHorzSegsToJoins`, `AddTrialHorzJoin`), which polyclip has not ported.

  **Measured: the standalone classification fix (1) is net-NEGATIVE.** Per-pair before/after diff over 2500 random simple-CCW-quad pairs (50%% snapped to share a vertex, coords [0,8], vs an MC oracle, fix toggled off vs on under the same seed): **fixed 0, regressed Union 1 / Intersect 3 / Difference 3, Xor 0.** End-to-end it fixes NOTHING (H4 stays 4.0, the larger repro 6.5→6.0, both still wrong) because (2) still cancels B. Worse, the regressions show the apex-Y skip is *wrong* for the **shared MAX/MIN-vertex** confluence (a vertex that is simultaneously one source's max and the other's min): there the other source's max edge apex IS at the scanline, so the skip excludes it, but Intersect/Difference DO need to count it (e.g. `Intersect((0,5),(5,5),(1,8),(1,6) ; (2,4),(2,3),(5,5),(-1,5))` shared (5,5): 0→6.5 wrong; the paired Difference 6.5→0 wrong). So the skip cannot be applied unconditionally — it would need to be narrowed to the genuine collinear-horizontal-overlap case, which is exactly what sub-bug (2) is about.

  **Conclusion:** the shared-collinear-horizontal class is a STRUCTURAL multi-session increment, not a one-session fix. It requires (1) correct winding classification at a collinear-horizontal max-plateau AND (2) collinear-horizontal overlap cancellation/handoff in `doHorizontal` (port Clipper2's horizontal-join machinery), done together and validated against the MC oracle. Do NOT land sub-fix (1) alone. **Net for this session: no source change (the partial fix is a confirmed net-negative); deliverable is this diagnosis.** Throwaway scaffolding (`clip/zz_trace.go` trace+toggle, `clip/zz_dump_test.go`, `zz_repro_test.go`, `zz_dump2_test.go`, `zz_cases_test.go`, `zz_diff_test.go`, and the `Classify`/`doHorizontal`/output trace calls) deleted before close. Method note: size the MC differential SMALL from the start (≤2500 pairs, ≤40k samples, ALL four ops from one sample loop) — see the [[differential-harness-sizing]] memory.

  **RESOLVED — and the "two coupled sub-bugs / structural multi-session" diagnosis above was a MISDIAGNOSIS (2026-05-21, this session, commit on `feat-horz-join`).** The shared-collinear-horizontal class is fixed by a **single one-line ordering change**; the horizontal-join port (sub-bug 2) is NOT needed. **Root cause: scanline phasing.** polyclip's `handleScanlineBound` ran the scanline's local-max **horizontal** plateaus too late — they were queued during the Tops phase (by `advanceBoundCursor`/`closeBound`) but flushed by `run()` only AFTER `handleLocalMin`. So A's closing max-plateau lingered in the AEL when B's coincident local min was classified, and B's `WindOther` left-walk counted it (B → `WindOther=1` → non-contributing → dropped). This is exactly sub-bug (1) — but it is an **ordering** bug, not a `Classify` bug. **Fix:** call `s.flushPendingHoriz(y)` once more inside `handleScanlineBound`, immediately after the Tops loop and before the local-minima loop. This mirrors Clipper2's ExecuteInternal phasing precisely: `DoTopOfScanbeam(Y)` + its `DoHorizontal` run at the END of one scanbeam iteration, BEFORE the NEXT iteration's `InsertLocalMinimaIntoAEL(Y)` at the same Y (engine.cpp:2127–2141). Min (leading) horizontals are still queued later in the local-minima loop and flushed by `run()` afterward, so their phasing is unchanged.

  **Why sub-bug (2) evaporated.** With the plateau removed before B is classified, A's plateau `doHorizontal` runs while B is not yet in the AEL — so the "spurious cross of B's left bound at the shared near vertex → `AddLocalMaxPoly` cancellation" never occurs. Sub-bug (2) was a *downstream symptom* of the wrong phasing (B was being created late and tangled), not an independent defect. **Proven empirically:** the full Clipper2 horizontal-join subsystem (`HorzSegment`/`HorzJoin`, `AddTrialHorzJoin`, `getLastOp`, `DuplicateOp`, `UpdateHorzSegment`, `ConvertHorzSegsToJoins`, `ProcessHorzJoins`, three `doHorizontal` trial-join sites, the per-scanbeam convert + end-of-sweep process hooks) WAS fully ported on a throwaway branch and gated behind a toggle; across H4, four hand-built collinear-overlap cases (stacked squares, partial-stacked, the larger `(9,4)…` repro, overlap-stacked), and a 3000-pair MC differential, the join was **completely inert** — `order-only` produced byte-identical results to `order+join` in every case. So the join port was deleted (no test or differential pair exercises it; landing inert machinery violates the "no abstractions beyond what the task needs" rule). If a future case ever genuinely needs collinear-horizontal splicing, re-port it then with a failing test to justify it.

  **Effect (3000-pair MC differential, 60%% shared-vertex, coords [0,8], tol 0.15, before→after, ZERO per-pair regressions):** Union 0.58%→0.33% (14→8 wrong), Intersect 0.17%→0.00% (4→0), Difference 0.50%→0.37% (12→9), Xor 0.37%→0.29% (9→7); per-pair: 6 Union / 4 Intersect / 3 Difference / 2 Xor fixed, **0 regressed**. H4 all four ops correct (Union 4.0→6.0); the larger repro `Xor((9,4),(0,4),(1,0),(1,3) ; (9,4),(4,11),(2,5),(8,4))` 10.0→28.5 (truth ~28.6) and its Union 6.5→28.5. Guard: `TestSharedCollinearHorizontalEdge` (boolean_test.go, all four ops on H4). Files: `clip/sweep.go` (+12, one `flushPendingHoriz` call + comment), `boolean_test.go`. The d50048a `AddLocalMaxPoly` workaround is untouched and still present (separate Xor coin-flip class). Throwaway scaffolding (root `zz_horz_test.go`/`zz_verify_test.go` MC differential with toggle, `clip/zz_trace_test.go` descale trace, the full ported `clip/horzjoin.go`, and the `zzf`/`zzDbg`/`ZZSetDisable` hooks) all deleted before commit.

  **Residual characterized — it is ONE class: cross-source PARTIAL collinear-overlap of horizontal edges, resolved into a fully-coincident horizontal pair by `SplitOverlaps` and then mis-classified by the sweep (2026-05-22, characterization-only, no code change).** A throwaway differential over **random simple CCW integer quads in [0,8]** (4000 interacting pairs/seed, 120k MC samples, all four ops in one sample loop, two seeds 20260521 & 7777) plus an **off-grid perturbation re-run** of every wrong pair gives a decisive, single-class result:
  - **Seed 20260521: 7 wrong pairs (Union 2, Difference 3, Xor 2), 100%% `collinear-overlap` class.** Seed 7777: 5 wrong (Difference 4, Union 1), again **100%% `collinear-overlap`**. No `shared`, `vertex-on-edge`, or `clean` failures at all. Crossing-count is spread (0–3), so it is not a general-crossing residual.
  - **Every wrong pair becomes CORRECT when all coords are nudged off-grid (structural=0, artifact=12 across both seeds).** So the residual is purely a snap/degeneracy effect, NOT a structural sweep/orientation bug — no OutRec-ownership port is implicated.
  - **The class is concretely: two horizontal edges from different sources that are collinear and PARTIALLY overlap** (e.g. seed-20260521 minimal cross=0 Difference `A=(3,4),(6,6),(7,8),(2,8) \ B=(3,0),(6,4),(6,8),(1,8)` got 0.0 vs truth ~0.99 — A's top edge `(7,8)-(2,8)` and B's top edge `(6,8)-(1,8)` overlap on x∈[2,6] at y=8). Confirmed end-to-end via a `SplitOverlaps` probe: preprocessing **correctly** splits the partial overlap into a **fully-coincident different-source horizontal pair** on the overlap interval. `DedupCoincidentEdges` then deliberately **leaves different-source coincident pairs to the sweep** (preprocess.go:178–224, §12.6.1: "resolved by the standard winding classification … no special handling required"). So the fault is downstream, in the **sweep's winding classification of a different-source fully-coincident HORIZONTAL pair** — the same path the 21dc50e `flushPendingHoriz` ordering fix touched, but a distinct remaining sub-case (21dc50e fixed the max-plateau-vs-coincident-local-min phasing; this is the coincident-horizontal-pair classification itself, often mid-bound rather than at a shared local min/max).
  - **Magnitudes are gross (not tolerance noise):** e.g. Union 6.75 vs 21.75, Xor 13 vs 21.5 — the sweep drops a whole region when the coincident horizontal pair is mis-classified. So although the *rate* is ~0.1–0.2%%, individual failures are large.

  **Partial-collinear-horizontal residual — NEITHER-HOT sub-case FIXED (2026-05-22, branch `fix-coincident-horizontal`): closeBound Case B now emits its own apex.** Tracing the two minimal repros (cross=0 Difference `A=(3,4),(6,6),(7,8),(2,8) ; B=(3,0),(6,4),(6,8),(1,8)` and Union `A=(1,2),(7,1),(8,3),(3,3) ; B=(0,2),(4,3),(8,3),(2,6)`) showed the diagnosis "wrong WindSelf/WindOther" was off — the windings are correct. The real fault is in **ring closure**, not classification. The coincident horizontal pair (created by `SplitOverlaps` from the partial overlap) makes the sweep build a **cross-source output ring whose two hot edges terminate at DIFFERENT points joined by a horizontal top** — e.g. for the Difference repro, ring or0 (the thin top-right triangle (6,6),(7,8),(6,8)) has A's right edge ending at (7,8) and B's right edge ending at (6,8), with A's top segment [6,7] spanning between. The two hot edges close at different scanline events via `closeBound`'s Case A (emit apex, leave coupling) / Case B (partner already ran Case A). **Case B closed the ring WITHOUT emitting its own apex** — correct for a same-source plateau where both bounds top out at the SAME point (Case A's vertex already sits on the ring), but for this cross-source ring the two apexes DIFFER, so dropping one collapsed the ring to a degenerate two-point sliver and the whole thin region was lost. **Fix (`closeBound`, sweep.go):** in Case B, emit `ae`'s apex via `AddOutPt` when `maxPt` differs from BOTH ends of the open chain (head and back OutPts); the guard skips the same-source-plateau duplicate (which `dedupConsecutive` would have stripped anyway). **Effect (per-pair before/after diff, Case-B-emit toggled off vs on, 16000 random simple-CCW-quad pairs [0,8] across 4 seeds, identical MC truth, gross tol 0.6): 16 gross failures fixed, ZERO regressions.** On the 6000-pair gross differential: Union 6→3, Difference 2→0, Intersect 0, Xor 5→5. Full suite green; guard `TestCoincidentHorizontalOverlapClosesRing` (both repros, all four ops on repro 1, Union/Intersect/Difference on repro 2). Method: throwaway clip-package descale trace (`clip/zz_trace.go`) + a Go-only MC differential with a `ZZNoCaseBEmit` toggle for the per-pair diff, all deleted before commit.

  **RECOMMENDED NEXT increment: the BOTH-HOT coincident-horizontal sub-case (the remaining Union/Xor residual).** The fix above handles coincident horizontals whose rings originate non-hot (cross-source ring built up to a max). The remaining gross failures (e.g. Union `A=(8,4),(3,4),(2,7),(2,1) ; B=(6,0),(8,4),(0,4),(1,3)` shared coincident horizontal [3,8] at y=4; Xor `A=(1,2),(7,1),(8,3),(3,3) ; B=(0,2),(4,3),(8,3),(2,6)` got 15.5 vs ~21.49) are the case where BOTH coincident horizontals are HOT. `doHorizontal` then crosses them via `IntersectEdges` at their shared NEAR endpoint (where they begin together, x=4 for the Xor repro) — a geometrically bogus "crossing" of two collinear-overlapping edges that fires the both-hot tunnel branch (`AddLocalMaxPoly`+`AddLocalMinPoly`) and tangles/cancels the rings. Two collinear horizontals do not cross transversally; the near-endpoint `IntersectEdges` is the wrong tool. A naive "break the walk on a collinear horizontal" was tried and is net-negative (it abandons real crossings beyond the coincident edge → Intersect regressed). This is the collinear-horizontal overlap handoff Clipper2 resolves with its `horz_seg_list_`/`ProcessHorzJoins` machinery (which polyclip ported and deleted as inert — but that inertness proof did NOT cover the partial-overlap→coincident-pair config; re-port with these failing cases as justification, or find the targeted handoff). Validate against the [0,8] simple-quad MC differential (NOT Clipper2; it drops these slivers at small scale). The d50048a `AddLocalMaxPoly` workaround is unrelated and stays until the structural OutRec-ownership track lands.

  **Coincident-horizontal cross — REFINED diagnosis + local-skip REFUTED (2026-05-22, this session, NO source change).** Traced both repros end-to-end (clip-package descale trace on `doHorizontal`/`IntersectEdges`/`closeBound`/`AddLocalMin`+`MaxPoly`) and discovered the class is actually **two distinct sub-cases**, not one:
  - **Anti-parallel, BOTH-HOT (Xor `A=(1,2),(7,1),(8,3),(3,3) ; B=(0,2),(4,3),(8,3),(2,6)`, got 15.5 vs 21.48; ONLY Xor wrong, U/I/D correct).** A's top edge `(8,3)→(4,3)` (interior below, traversed −x) and B's bottom edge `(4,3)→(8,3)` (interior above, traversed +x) are anti-parallel over the coincident span [4,8]. The trace shows they are **already FRONT and BACK of the SAME OutRec** (a degenerate zero-width ring — the pinched A∩B overlap sliver) before `doHorizontal` runs. `doHorizontal` then "crosses" them at the near endpoint (4,3) via the both-hot tunnel (`e1.Outrec==e2.Outrec` ⇒ `AddLocalMaxPoly`+`AddLocalMinPoly`), TWICE (once per horizontal's pass), tangling. The real boundary needs A's right edge to hand off to B's upper edge `(8,3)→(2,6)` ACROSS the (interior) span — a non-local re-splice.
  - **Parallel, ONE-HOT (Union `A=(8,4),(3,4),(2,7),(2,1) ; B=(6,0),(8,4),(0,4),(1,3)`, got 17.89 vs 19.37; ONLY Union wrong).** Both top edges (both interior BELOW, both traversed −x) coincide over [3,8]; exactly one is hot (the contributing boundary), the other is a cold same-side duplicate. `doHorizontal` crosses them one-hot at the near (3,4) → `SwapOutrecs` wrongly transfers the ring to the cold duplicate.
  - **A local `doHorizontal` skip is provably net-negative however gated.** Tried "step horz past a coincident collinear horizontal without `IntersectEdges`" (zero-width ⇒ winding unchanged): it fixes the parallel one-hot Union repro exactly (19.39) but, measured over a 30000-op differential (2500 simple-CCW-quad pairs [0,8] × 3 seeds × 4 ops, tol 0.6, per-pair on/off diff): **unconditional skip = 0 fixed / 87 regressed; restricting to fully-coincident-AND-exactly-one-hot = 0 fixed / 46 regressed.** The regressions are one-hot coincident crosses that are LEGITIMATELY needed — e.g. at a y=1 bottom-edge local min `A=(0,2),(1,1),(6,1),(8,1) ; B=(3,5),(3,1),(6,1),(8,7)` skipping drops Union 21.18→5.32. So there is **NO local discriminator** (mirrors the d50048a same-side finding): whether a coincident-horizontal cross must splice vs must cross depends on the global ring topology, exactly what Clipper2's `horz_seg_list_`/`ConvertHorzSegsToJoins`/`ProcessHorzJoins` decides. The current-main residual for this whole class is ~8 gross failures / 30000 ops (~0.027%, `{I:1,U:2,X:5}`), all of it coincident-horizontal degeneracy.
  - **Conclusion:** the both-hot (and parallel one-hot) coincident-horizontal class is a STRUCTURAL multi-session increment requiring the horizontal-join port — including the OutRec **splits/owner** machinery polyclip does not yet have (`ProcessHorzJoins`'s same-OutRec case does `NewOutRec`+`FixOutRecPts`+owner/`MoveSplits` tracking). Do NOT land a local `doHorizontal` skip (net-negative). Net for this session: no source change; deliverable is this refined two-sub-case diagnosis + the local-skip refutation. Throwaway scaffolding (`clip/zz_trace.go` descale trace + `ZZSkipCoincidentHorz` toggle, root `zz_repro_test.go`/`zz_diff_test.go` MC differential) all deleted before close.

  **Architecture RESOLVED — the splits/owner machinery is NOT needed (2026-05-21, read-only investigation, no source change).** The conclusion just above ("requires the OutRec splits/owner machinery") is **CORRECTED**: it is over-cautious and wrong. Reading Clipper2 `ProcessHorzJoins` directly (engine.cpp:2268–2334) shows the `splits` list, `MoveSplits`, `CheckSplitOwner`, and the `Path2ContainsPath1` containment swaps are ALL inside `using_polytree_` guards (2295–2317, 2324–2329). polyclip is **non-polytree** — it computes hole nesting in postprocess (`assembleResult`/`interiorPoint`), so the non-polytree branch of `ProcessHorzJoins` is all polyclip needs, and it reduces to: (i) the unconditional OutPt-chain splice (2275–2280); (ii) same-ring "join is really a split" → `NewOutRec` + `FixOutRecPts` + repair `or1->pts` (2284–2293, 2318–2319); (iii) different-ring "join" → `or2->pts = nullptr` release (2321, 2330–2331). The non-polytree `or2->owner = or1` assignment is dead for polyclip (postprocess recomputes ownership). **Empirically confirmed:** the `feat-horz-joins` branch's join port (`clip/horzjoin.go`, a faithful transcription with NO splits/owner) already FIXES the `(c)` Union repro (`A=(4,4),(5,4),(7,3),(1,7) ; B=(0,2),(3,2),(5,4),(2,4)` 6.0→8.49) on the [0,8] MC differential with zero Intersect/Xor regression. So the recurring "needs splits/owner / multi-session structural rework" framing across the 2026-05-22 notes is retired.

  **Recommended architecture: keep-split + gated-skip + the join port (NOT don't-split).** Of the two ways to stop `doHorizontal` from bogusly crossing a coincident collinear-horizontal pair: (a) *don't-split* — drop `SplitOverlaps`' horizontal-overlap splitting to match Clipper2's whole-horizontal model — is REJECTED: it requires reworking the sweep's winding classification + ring construction for WHOLE overlapping horizontals (polyclip is built on the no-overlap invariant; the don't-split experiment regressed the differential U2→11), the genuine large "piece-2 bulk". (b) *keep-split + gated-skip + join port* (the `feat-horz-joins` branch) is VIABLE and far smaller — it already fixes `(c)` Union. **Concrete remaining work on that branch** (current ON-toggle differential U=2 D=2): (1) **run-continuity** — `(0,2),(2,3),(3,3),(1,8) ; (0,3),(3,3),(8,2),(6,7)` Union 5.40 vs 23.16: a mid-scanbeam ring merge leaves an OutPt-chain discontinuity at the overlap boundary so `convertHorzSegsToJoins` can't capture the A/B overlap as two separable runs — restore chain continuity or anchor the trial op correctly under the skip; (2) **one-hot Difference path** — the gated skip lives only in `branchBothHot`; add the analogous gating to the one-hot path (gate it `!samePolyType && op!=OpXor` + coincident-collinear-horizontal + w≤1 + different ring, mirroring the both-hot skip — the broad ungated one-hot variant regressed I0→18); (3) **`(d)`** `A=(5,1),(6,5),(5,5),(1,7) ; B=(1,1),(7,0),(7,3),(6,5)` — characterize after 1+2. Validate against the MC oracle (NOT Clipper2); delete all `zz_*`/toggle scaffolding before merge.

  **LANDED — coincident-horizontal opposite-side cancellation (keep-split + discriminated skip + horz-join port).** The fix ships in the split model (no don't-split). Root, confirmed against instrumented Clipper2 across `(c)`/the run-continuity regression/a top-overlap rectangle: when two collinear horizontals from different sources overlap, polyclip's `SplitOverlaps`/`SplitTJunctions` manufacture a coincident pair that `IntersectEdges` then *crosses* — geometrically bogus, since overlapping horizontals do not cross transversally. The decisive discriminator (which earlier notes wrongly concluded did not exist) is the pair's **interior sides**, read off `Segment.Reversed`:
  - **opposite interiors (`Reversed` differs)** — the shared edge is interior (Union cancels it; Difference/Intersect keep one source's boundary). NOT a crossing. `dispatchIntersect` now returns nil for a coincident (`max(Bot.X) < min(Top.X)`), opposite-interior, `w≤1`, different-ring, non-Xor horizontal pair — both-hot AND one-hot. Each bound keeps building its own clean run and `processHorzJoins` reconnects them. This fixes `(c)` Union (6.0→8.5, one merged ring identical to Clipper2) AND `(c)` Difference (0→2.5), and the discriminator lets the run-continuity regression `(0,2),(2,3),(3,3),(1,8) ; (0,3),(3,3),(8,2),(6,7)` fall through to the (already-correct) same-side path.
  - **same interiors (`Reversed` equal)** — a genuine doubled boundary where one edge is interior; falls through to normal dispatch (baseline-correct).
  The horz-join port (`clip/horzjoin.go`) runs for all non-Xor ops; **Xor is excluded** (its coincident pairs are resolved by the standard maximum handling, and an unrestricted `processHorzJoins` infinite-loops on some Xor inputs — the join's purpose is to reconnect skip-separated runs, which only occur for non-Xor). Effect on the [0,8] simple-quad MC differential: 4 gross fails → 2 (U2 D2 → U1 D1), zero regressions, full suite green. Regression test `TestCoincidentHorizontalOppositeSideCancels`. **Remaining (separate class, pre-existing):** `(d)` `A=(5,1),(6,5),(5,5),(1,7) ; B=(1,1),(7,0),(7,3),(6,5)` (U/D) — A's local min `(5,1)` is interior to B, and A's right bound must go hot when it exits B at the shared vertex `(6,5)`, but that shared-vertex crossing isn't dispatched. This is the shared-vertex track, NOT coincident-horizontals.

  **LANDED — input orientation normalization at the engine boundary (2026-05-21).** Independent increment, flagged pending across many sessions. The sweep derives each edge's `WindDx` purely from edge direction (`clip.signedContribution`: `+1` if `Reversed` (descends in ring order), else `-1`), so it ASSUMES a canonical input winding (CCW outer, CW hole). Inputs with the opposite winding invert `WindDx` for that source and misclassify. Demonstrated bug: a CW subject + CCW clip gave Union 26 / Xor 22 instead of 28 / 24 (the four orientation combos of two overlapping squares were not all equal). `runBooleanOp` never enforced or normalized orientation, contradicting `validate.go`'s `IssueWrongWinding` doc that claimed "the engine normalises internally" — the doc was WRONG. Fix: `appendRing` (boolean.go) now walks the ring in reverse when its signed area disagrees with the wanted orientation (`reverse := (ring.SignedArea() < 0) == wantCCW`; `collectSegments` passes `wantCCW=true` for outers, `false` for holes). A no-op for canonical input (so the all-CCW MC differential is unaffected), and makes the `validate.go` doc accurate. Verified: all four orientation combos → equal results; a holed polygon yields the same Intersect (20.0) with a CW or CCW hole through the engine. Guard `TestInputOrientationNormalized` (boolean_test.go). Files: boolean.go (+10), validate.go (doc), boolean_test.go.

  **RESIDUAL re-characterized (2026-05-21, throwaway [0,8] MC differential, 3 seeds × 5000 pairs, 100k samples, tol 0.6).** 17 gross fails (U6 I0 D2 X9 across 12535 interacting pairs, ~0.05–0.1%); **100% become correct under off-grid perturbation** → entirely on-grid degeneracy, NO structural bug. Two classes, both freshly traced through the full ring lifecycle, both the documented multi-session structural reworks (no tractable single-session local fix — consistent with the long history of refuted local patches):
  1. **Collinear-horizontal overlap (~65%, the grossest: e.g. Difference `A=(4,3),(2,6),(7,6),(0,8) ; B=(5,3),(3,6),(0,6),(3,4)` got 0.042 vs 5.458).** The c9d8a10 opposite-side SKIP fires correctly, BUT a nearby bottom crossing closes A's main ring early via `AddLocalMaxPoly`, and the horizontal overlap produces only **1 live run** in `convertHorzSegsToJoins` (needs ≥2 to pair) → `processHorzJoins` does 0 joins → A's body collapses to slivers. This is the documented "run-continuity" blocker; the genuine fix is the whole-horizontal `doHorizontal` rework (don't-split + whole-horizontal handling + horz-join firing on whole horizontals).
  2. **Vtx-on-edge same-side tangle (~35%, Xor-heavy: e.g. Xor `A=(2,2),(7,2),(5,5),(3,5) ; B=(0,0),(7,7),(3,4),(2,6)` got 13.2 vs 14.44; U/I/D pass).** A's vertices (2,2) and (5,5) both lie on B's diagonal (0,0)-(7,7); the final `AddLocalMaxPoly` fires with both edges `front=false` (the d50048a same-side workaround case) → a tangled self-touching ring visiting (5,5) 3×. Needs the OutRec front/back ownership port (crossing-spawned-ring orientation correctness).

  **NEXT increment recommendation: the whole-horizontal `doHorizontal` rework** (class 1 — the larger share and the higher-value real-world case: adjacent polygons sharing a horizontal boundary). It is a genuine multi-session atomic effort with no green partial checkpoint (every documented partial measure regressed the differential), so it should be sequenced as a dedicated multi-session increment, not started half-way under time pressure.

  **LANDED — coincident-horizontal EXIT discriminator: the run-continuity class is fixable WITHIN the shipped keep-split + skip + join architecture (no don't-split rework needed) (2026-05-21).** The "run-continuity blocker" above was re-traced and turned out to have a clean LOCAL discriminator after all. Mechanism (Difference repro `A=(4,3),(2,6),(7,6),(0,8) ; B=(5,3),(3,6),(0,6),(3,4)`, was 0.042 vs ~5.44): `SplitOverlaps` fragments A's top horizontal `(2,6)-(7,6)` into `(2,6)-(3,6)` + `(3,6)-(7,6)` over its overlap [2,3] with B's top edge. At the overlap, B's bound ENDS (it is B's local-max plateau apex — the coincident piece is its last segment) but A's bound CONTINUES collinearly to `(7,6)`. The c9d8a10 opposite-side skip fired here and dropped A's continuation: A's right bound never re-spawned, so when A's left bound reached the apex `(0,8)` it had no hot partner and the whole upper body collapsed. **Discriminator (`continuesCollinearHorizontal`, clip/poly_ops.go):** a coincident opposite-side pair is a mutual cancellation only when NEITHER bound continues collinearly past the overlap; if exactly one continues (its next bound segment is a collinear horizontal at the same Y), the pair is a boundary EXIT and must fall through to the normal one-hot dispatch (which transfers the ring onto the continuing cold bound so it re-spawns). The narrow "collinear-horizontal continuation" form is essential: a broader "continues with ANY segment" test regressed 3 Union pairs (genuine cancellations where both bounds continue past). Effect on the [0,8] simple-quad MC differential (3 seeds × 5000 pairs, tol 0.6): **15 → 11 gross fails (3 Union + 1 Difference fixed, ZERO regressions)** — every remaining fail was already failing pre-fix. The U/I/D area identities now hold exactly for the repro (U 10.4417 + I 0.5583 = |A|+|B| = 11; D = |A|−I). Guard `TestCoincidentHorizontalExitReSpawns`. The remaining residual is class 2 (vtx-on-edge same-side tangle, Xor-heavy, needs the OutRec front/back ownership work) plus one shared-vertex-corner Difference (`(7,6),(8,6),(8,8),(1,3) ; (1,2),(2,0),(8,6),(1,6)` — A turns vertical at the shared apex `(8,6)`, a distinct class the collinear-horizontal discriminator deliberately does not catch). So the "whole-horizontal rework" recommended above is NOT needed for the run-continuity class — keep-split + skip + join + this discriminator handles it.

  **LANDED — coincident-horizontal CORNER EXIT (continuation leaves non-horizontally) (2026-05-22).** The "shared-vertex-corner Difference" case the previous note deliberately did not catch (`A=(7,6),(8,6),(8,8),(1,3) ; B=(1,2),(2,0),(8,6),(1,6)`, Difference 0.0 vs MC 2.80) is the SAME boundary-EXIT mechanism as the horizontal-exit above, except A's continuation past the overlap is VERTICAL (`(8,6)→(8,8)`), not a collinear horizontal — so `continuesCollinearHorizontal` returns false and the opposite-side skip wrongly fired, dropping A's upper region. A's top horizontal `(7,6)-(8,6)` overlaps B's top `(8,6)-(1,6)` over [7,8]; at the shared apex `(8,6)` B's bound ENDS (its local-max plateau) while A continues up. Traced both this case and the passing cancellation `TestCoincidentHorizontalOppositeSideCancels` side by side at the dispatch: BOTH have "exactly one bound continues past the overlap", so that alone does not discriminate. The decisive difference is **hotness**: in the corner-exit the ENDING bound (B) is hot and the CONTINUING bound (A) is cold, so the one-hot dispatch's `AddOutPt+SwapOutrecs` transfers the ring onto A's continuing cold bound = re-spawn (correct); in the cancellation the continuing bound is already hot, so the ring is already on the right bound and the skip must fire (transferring would wrongly move it onto the ending bound). **Discriminator (`respawnHandoffAtOverlap`, clip/poly_ops.go):** added as an EXTRA don't-skip exception (alongside `continuesCollinearHorizontal`) — exactly one bound is `IsBoundLast` (ends at its apex plateau), the ending one is hot, the continuing one is cold. Because it only ADDS a don't-skip case (never removes a skip for the both-continue cancellation class, where neither bound is last), it is strictly narrower than the rejected "continues with ANY segment" form and cannot regress those. Effect on the [0,8] simple-quad MC differential (5 seeds × 6000 pairs, 90k samples, tol 0.6, per-pair before/after via a `ZZDisableRespawnHandoff` toggle): **3 Difference pairs fixed, ZERO regressions across all ops/seeds.** The repro's Xor (got 22.1 vs 23.1) is a SEPARATE pre-existing class (Xor never runs the skip path) and is unaffected — left for the OutRec front/back work (class 2). Guard `TestCoincidentHorizontalCornerExitReSpawns`.

   **LANDED — coincident-horizontal BOTH-CONTINUE: the skip now requires at least one `IsBoundLast` (2026-05-22).** A fresh [0,8] simple-quad MC differential (15 seeds × 400 pairs, 120k samples, tol 0.6) over the post-`7881225` engine surfaced just 3 gross fails (~0.17% of 1744 interacting), all coincident/collinear-horizontal on-grid degeneracy (every fail is correct off-grid). The grossest was `A=(0,3),(6,5),(8,5),(2,8) ; B=(0,4),(8,5),(7,5),(2,6)`: A's bottom horizontal `(6,5)-(8,5)` and B's top horizontal `(8,5)-(7,5)` overlap over [7,8] at y=5 (interiors opposite, shared vertex `(8,5)`), and **Union (16.52→4.69) and Intersect (5.98→1.81) collapsed while Difference and Xor were already correct**. Dumping the rings showed the engine's Xor output already contains the correct union outline (outer) AND the correct intersection region (hole) — so the geometry is computed fine; only the U/I dispatch was sealing both rings along y=5. The opposite-side skip was firing even though **neither bound terminates at the overlap**: A continues up to `(2,8)`, B up to `(2,6)`, both with sloped edges. A genuine doubled-boundary cancellation has one polygon's bound turning at the shared edge (its plateau ends there, `IsBoundLast`); when both bounds carry on past the overlap the coincident horizontals are two live boundaries that must be dispatched normally, not skipped. **Discriminator: add `(e1.IsBoundLast() || e2.IsBoundLast())` to the skip condition.** This keeps the cancellation guard (`TestCoincidentHorizontalOppositeSideCancels`, where e1 is a `last` plateau) firing and is strictly narrower than before — disabling the skip entirely was measured to give 0 regressions over 2000 random pairs and only that one guard depended on it, so a narrowing between full-skip and no-skip is safe. Per-pair before/after over 6000 pairs (15 seeds): **2 fixed (the U+I above), ZERO regressions across all ops.** The remaining residual is the Xor-only / vtx-on-edge classes (e.g. `A=(4,1),(6,6),(3,6),(0,6) ; B=(2,4),(3,2),(8,7),(4,5)` Xor — A has a collinear mid-vertex `(3,6)` on its horizontal top), which never run the non-Xor skip path. Guard `TestCoincidentHorizontalBothContinueNoSkip`. NOTE: the "BuildLocalMinima near-hang at 1200+ pairs" flagged in earlier resume notes was a harness MC-oversizing artifact — 16000 pairs run with no MC sampling complete in ~18s with no individual slow input; there is no engine hang.

   **LANDED — vertex-on-edge Intersect self-close + Xor apex figure-8 merge (2026-05-22).** Two independent vtx-on-edge fixes, validated against a Monte-Carlo area oracle (Xor == Union − Intersect); Clipper2 is NOT a usable reference here — at native [0,8] integer scale it ROUNDS fractional edge intersections (e.g. (5.333,4.333)→(5,4)) and is wrong on all four ops, and pre-scaling its input by 1e6 confirms the MC values (X-midvtx Xor 10.278 as four separate rings). (1) **Intersect self-close** (`closeBound`, clip/sweep.go): when a bound maxes out at a shared through-vertex and its OWN ring's coupled cross-source edge continues above but reclassifies as non-contributing (op-region ends here), the ring must close and the coupled edge continue cold; otherwise the continuation drags a spurious sub-loop in and the Intersect region cancels (`A=(0,0),(4,4),(8,5),(1,8) ; B=(2,2),(5,2),(3,3),(0,3)` Intersect 2.97→1.47; U/D/X already correct). (2) **Xor apex figure-8** (`AddLocalMaxPoly`, clip/output.go): polyclip's bottom-up sweep builds two interleaved rings that meet SAME-side at a maximum apex where Clipper2's top-down sweep would close a ring on itself; the d50048a relabel+`JoinOutrecPaths` recovery cannot splice two same-front chains and folds the apex into a degenerate out-and-back spike, losing the apex region's area (`A=(4,1),(6,6),(3,6),(0,6) ; B=(2,4),(3,2),(8,7),(4,5)` Xor 8.94→10.28, the documented collinear-mid-vertex (3,6) case; `A=(4,1),(7,3),(5,4),(4,3) ; B=(3,4),(5,4),(7,2),(8,8)` Xor 4.6→14.10). When neither ring has a CONTINUING other edge (both other sides already left the AEL — a terminal vtx-on-edge maximum), `AddLocalMaxPoly` now merges the two rings as a figure-8 PINCH (emit the apex on each, cross-link the two apex OutPts into one self-touching cycle) and `assembleResult`'s new `splitSelfTouchingRings` decomposes that walk into the correct simple rings at every revisited vertex — no front/back orientation guess needed. Rings with a continuing other edge (the tunnel/different-polytype joins) keep the relabel+`JoinOutrecPaths` fallback. Over an [0,8] simple-CCW-quad MC differential (8 seeds × 1000 pairs × 4 ops = 32000 evaluations, tol 0.6, self-intersecting quads rejected so even-odd MC == nonzero engine): both fixes drop the gross-fail rate to 4/32000 (~0.012%), all 4 PRE-EXISTING other-class failures (verified via a figure-8 on/off toggle — none are regressions). Full suite green, lint 0. Guards `TestIntersectVertexOnEdgeSelfClose`, `TestXorVertexOnEdgeApexMerge`.

   **LANDED — traceRing open-ring guard (2026-05-22).** `BuildLocalMinima`'s `traceRing` (clip/bounds.go) walks input segments following End→Start links until it returns to `start`; `pickNextSegment` deliberately includes already-visited candidates so the loop can detect closure. On a self-touching / self-intersecting input it could step into a visited NON-start segment and follow a sub-cycle that never reaches `start`, growing the ring slice without bound → memory exhaustion (an OOM that took down the dev environment, not just a hang). Fix: a visited non-`start` next means no simple ring closes — return `ErrOpenRing`. Bounds the walk to ≤ len(segs); the error propagates through `runBooleanOp`. (This is the "BuildLocalMinima near-hang" earlier notes mis-attributed to harness MC-oversizing — it is a real pre-existing engine bug, surfaced by a random-quad differential that did not reject self-intersecting quads.)

   **LANDED — notch-tip maximum on a crossing horizontal: `handoffMaxThroughVertex` defers to a genuine same-source maximum (2026-05-22).** Worst gross fail of the post-figure-8 residual (`U A=(6,5),(7,3),(7,8),(3,3) ; B=(8,5),(2,6),(3,0),(5,5)` got 3.38 vs MC 14.29 — d≈11; I/D/X already correct). A's vertex `(6,5)` is a concave-notch local **maximum**: A's two lower edges `(3,3)-(6,5)` and `(7,3)-(6,5)` both end there, so A's source CLOSES a maximum at the apex. B's horizontal edge `(5,5)-(8,5)` passes exactly through `(6,5)` (`SplitTJunctions` splits it into `(5,5)-(6,5)` + `(6,5)-(8,5)`). For Union both A-edges are hot at the apex, so `closeBound` ran `handoffMaxThroughVertex` FIRST (before `maximaPartner`) and saw B's cold horizontal piece as a through-edge "continuing above" (B's bound apex `(2,6)` is higher); it crossed A's hot edge with the horizontal via `IntersectEdges`, whose one-hot `SwapOutrecs` moved A's hot ring onto the horizontal — leaving A's edge COLD and no longer AEL-adjacent to its notch partner, so the genuine maximum never closed via `AddLocalMaxPoly` and the rings shattered. I/D/X are unaffected because A's notch edges are not hot there (the region below the notch tip is not on their boundary). **Fix (`handoffMaxThroughVertex`, clip/sweep.go):** return early when `ae` has a same-source maxima partner at `maxPt`. A genuine maximum is closed by the `maximaPartner`/`resolveBetweenMaxima` path, which crosses true between-edges itself; the standalone hand-off is only for a vertex-on-edge EXIT where `ae`'s source has no second edge ending at `maxPt`. This subsumes the §"Shared-vertex EXIT via a horizontal" case (`TestSharedVertexExitViaHorizontal` still passes): there A's through-edge is genuinely *between* B's two co-maximum edges, so `resolveBetweenMaxima` crosses it and the hand-off effect happens through the maxima machinery instead. **Validated against a Monte-Carlo area oracle** (the worst case fixed 3.38→14.29; over a [0,8] simple-quad MC differential, 12 seeds × 1500 pairs incl. non-convex quads, ~3435 interacting, tol 0.6, per-pair before/after toggle: **0 regressions**; the degenerate config is ~0.012% so it rarely appears in random streams). Guard `TestUnionNotchTipOnHorizontalEdge`. Files: clip/sweep.go (one guard), boolean_test.go.

   **LANDED — coincident max-plateau over a continuing horizontal: `plateauPartnerPending` defers only to a partner that itself tops out at the apex (2026-05-22).** Xor gross fail (`X A=(0,8),(3,0),(2,4),(8,4) ; B=(0,4),(1,1),(5,3),(4,4)` got 25.37 vs MC 21.38 — d≈4; U/I/D already correct). B's local-max **plateau** `(0,4)-(4,4)` (y=4) is coincident with A's **mid-bound** horizontal `(2,4)-(8,4)` (also y=4, but A continues up to `(8,4)` then its apex `(0,8)`) over x∈[2,4]. `SplitOverlaps` cuts B's plateau at `(2,4)`, so its piece `(2,4)-(4,4)` tops out at `(4,4)`. When that piece reached `closeBound` at `(4,4)`, `plateauPartnerPending` returned true and DEFERRED the maximum — because the coupled OutRec partner (A's continuing horizontal piece, still in `pendingHoriz`, far-X `==` apex X) looked like a shared-plateau partner. But a *continuing* horizontal's `doHorizontal` promotes its cursor past the apex and never calls `closeBound` on the deferred edge, so B's plateau edge stayed HOT in the AEL all the way up to A's apex `(0,8)`. There it sat *between* A's two ascending bounds (off the apex column, x=2 ≠ 0), so `scanMaximaPartner` hit it, saw `XAtY ≠ maxPt.X`, and bailed → A's genuine maximum at `(0,8)` never paired. Each A bound then closed independently via the Case-A/B handoff, emitting **two overlapping outer rings** (the geometric membership was correct — MC of the result region = 21.34 — but `Area()` double-counted the ~4 overlap). **Fix (`plateauPartnerPending`, clip/sweep.go):** defer only when the coupled horizontal partner genuinely tops out at `maxPt`; if it CONTINUES above `maxPt` (`boundContinuesAbove`), drop the deferral so the maximum closes here (the cross-source self-closure / Case-A handoff then removes the edge and the apex closes via `AddLocalMaxPoly`). **Validated against a Monte-Carlo area oracle** (Xor == U−I; Clipper2 rounds these fractional intersections at native scale and is not a usable reference). Over a [0,8] simple-quad MC differential incl. non-convex quads (random-permutation + segment-crossing simplicity test + winding-normalize generator), 16 seeds × 800 pairs × 4 ops across two seed sets (~12800 pairs, 60k samples, tol 0.6, per-pair before/after toggle): **2 fixed, ZERO regressions** (the degenerate config is rare in random streams, so the known repro is tested explicitly). Guard `TestXorCoincidentMaxPlateauOverContinuingHorizontal`. Files: clip/sweep.go (one guard), boolean_test.go.

   **LANDED — shared-vertex concave maximum with a through-edge that turned onto a horizontal: `scanMaximaPartner` additionally accepts a continuing-above horizontal between-edge (2026-05-22).** Union/Difference region-drop at a vertex of A coincident exactly with a vertex of B. Repro `A=(3,3),(7,6),(6,6),(1,8) ; B=(7,7),(4,5),(7,6),(8,4)` (shared vtx `(7,6)`): **Union collapsed 13.22→2.66, Difference 11.22→0.78**; Intersect/Xor already correct. `(7,6)` is B's concave-vertex local MAXIMUM (B's edges `(4,5)-(7,6)` and `(8,4)-(7,6)` both end there) AND A's through-vertex (A turns from `(3,3)-(7,6)` onto its top horizontal `(7,6)-(6,6)`). A's through-edge sits AEL-*between* B's two max edges, so B's maximum should pair and `resolveBetweenMaxima` should cross A — updating A's `WindOther` 0→1 as A enters B's interior across the concave vertex. But A had already advanced onto its horizontal, and `scanMaximaPartner`'s apex-column gate was `XAtY(cand.Seg, maxPt.Y) == maxPt.X` — for a horizontal `XAtY` returns its `Bot.X` (x=6, the wrong end), so the scan bailed, B's max never paired, and A's `WindOther` stayed stale at 0. For Union/Difference (contribution depends on `WindOther`) A's body dropped; Xor was correct because it ignores `WindOther`. **Fix (`scanMaximaPartner`, clip/sweep.go):** keep the base `XAtY == maxPt.X` test and ADDITIONALLY accept a between-edge that is horizontal, spans the apex column (`throughVertexOnColumn`), AND whose bound continues strictly above `maxPt` (`boundContinuesAbove`). The continues-above guard is essential: a horizontal that itself tops out at the apex is part of a coincident max-PLATEAU, not a through-edge — widening pairing across it regressed coincident-horizontal cases (committed FuzzUnion/FuzzIntersect seeds went red and a FuzzDifference seed hung in `SplitOverlaps`). The clause is purely ADDITIVE (it never rejects a between-edge the base test accepted), so it cannot break the existing `TestUnionCoincidentHorizConfluence` whole-AEL pairing. **Validated against a Monte-Carlo area oracle** (all of U=X+I, D=|A|−I, X=U−I hold for the repro; NOT Clipper2). A per-pair before/after toggle confirmed the change leaves the coincident-horizontal fuzz seeds UNCHANGED (ON==OFF) while fixing both repro families; differential over 75000 non-convex-quad pairs at coordinate half-extents 8 and 40, **ZERO regressions** (the config is rare in random streams, so both families are tested explicitly). Full suite incl. all fuzz seed corpora green. Also fixes Family 2 (`A=(6,5),(3,4),(1,7),(4,0) ; B=(4,5),(0,7),(3,2),(6,5)`, shared vtx `(6,5)`). Guard `TestSharedVertexConcaveMaxThroughHorizontal`. Files: clip/sweep.go (one additive clause), boolean_test.go.

   **LANDED — `SplitOverlaps` infinite loop on a collinear spike: `collinearOverlap` returns exact endpoints instead of a rounded re-projection (2026-05-22).** A CI-wedging infinite loop (the most dangerous of the residual fuzz failures), originally `FuzzDifference/a84b6e584bd0aa6b`: `A=(-10,-10),(67,67),(10,10),(-99,10) ; B=(-3,-3),(3,-3),(3,76),(-3,3)`. A is a "spike" quad — its vertices `(-10,-10),(67,67),(10,10)` are collinear on y=x, so edge `(-10,-10)-(67,67)` and edge `(67,67)-(10,10)` overlap collinearly (a degenerate but non-self-*intersecting* input that passes the fuzz simplicity filter). `SplitOverlaps` loops calling `findFirstOverlap`/`splitAt` until no partial overlap remains. The overlap interval's lower endpoint is geometrically exactly the vertex `(10,10)`, but `collinearOverlap`'s sloped/vertical branch recomputed its X with `xAtY(a, ylo)` — a float projection onto the line that ROUNDS, landing it ~32 fixed-point units off the shared line. `splitAt` then cut there, and because the rounded point is off-line at the same Y as the endpoint it manufactured a spurious tiny **horizontal sliver** with `Bot.X > Top.X` (non-canonical). `splitAt` assumes its segment has `Bot < Top` in `LessYX` order, so on the non-canonical sliver it emitted three mutually-overlapping pieces, the overlap was never actually resolved, and the work slice grew by two every iteration forever (OOM under load). **Root cause:** for two collinear segments the overlap endpoints are always two of the four *exact* input endpoints (the inner two) — there is no need to re-project, and re-projecting introduces the rounding that breaks `splitAt`'s ordering invariant. **Fix (`collinearOverlap`, clip/intersect.go):** take the overlap endpoints directly from the input endpoints — `p` is whichever of `a.Bot`/`b.Bot` has the higher `Bot.Y` (achieves `ylo`), `q` is whichever of `a.Top`/`b.Top` has the lower `Top.Y` (achieves `yhi`). The horizontal-horizontal branch already used exact `min`/`max` X and was unaffected. `xAtY` had no other caller and was removed. **Validated:** the seed completes in 0.003s (was an infinite loop); all four committed fuzz seed corpora run, and the three remaining pre-existing failures (`FuzzUnion/a61d9e1c973bf217`, `FuzzIntersect/ff9aee9b909462b0`, `FuzzXor/28f7e421c4b5068b` — the coincident-horizontal class) have byte-identical areas before/after, so the change is regression-free; full clip + fixed suites green, lint 0. Guard `TestSplitOverlapsCollinearSpikeNoHang` (all four ops on the spike input). Files: clip/intersect.go (exact-endpoint selection, `xAtY` removed), boolean_test.go.

   **LANDED — redundant collinear-through input vertex: `appendRing` simplifies collinear vertices before the sweep (2026-05-22).** Two of the three remaining pre-existing fuzz seeds, both the coincident/collinear-horizontal class, shared one root: a ring with a vertex lying EXACTLY on the straight line between its neighbours (a geometrically redundant collinear-through vertex). The bound model treats the extra segment's shared endpoint as a turn/maximum of its bound, so a flat-topped ring whose top edge carries a collinear mid-vertex mis-builds its rings — especially when that plateau's end touches the other polygon's edge (a vertex-on-edge confluence). (1) `FuzzUnion/a61d9e1c973bf217` `A=(143,-5),(5,7),(9,5),(-60,5) ; B=(-7,5),(-19,5),(-33,5),(5,-7)`: B's top `(-7,5),(-19,5),(-33,5)` is collinear on y=5; Union UNDER-counted 369.50 (truth ~563) and Difference 213.50 (truth ~407) — A's body dropped. (2) `FuzzXor/28f7e421c4b5068b` `A=(-14,5),(97,-70),(5,5),(-5,5) ; B=(5,-34),(15,49),(105,107),(5,73)`: A's top `(5,5),(-5,5),(-14,5)` is collinear on y=5 AND A's plateau corner `(5,5)` lies exactly on B's vertical edge x=5; Xor OVER-counted 4182.50 (truth ~2522) as two overlapping outer rings — B's top maximum `(105,107)` arrived with its two edges on DIFFERENT rings (a same-side `AddLocalMaxPoly` figure-8 that decomposed into a geometrically-wrong oversized triangle poking outside B). A throwaway sweep trace confirmed: with the redundant vertex removed, B's top maximum becomes a clean same-ring opposite-side close. **Root cause:** a collinear-through vertex adds no geometry but creates a spurious extra segment that the Vatti bound model cannot reconcile. **Fix (`appendRing` / `simplifyCollinearRing`, boolean.go):** remove vertices with `fixed.Orient2D(prev, cur, next) == 0` from each INPUT ring before emitting segments — for a simple (non-self-intersecting) polygon every collinear vertex is a redundant through-vertex, so removal is an exact geometric no-op. Done on the input, BEFORE `SplitOverlaps`/`SplitTJunctions` introduce their own intended collinear split vertices at crossings. A wrap-around dedup (first == last) runs first so a ring with a repeated vertex never presents a zero-length neighbour that `Orient2D` would read as collinear and wrongly delete a real corner. **Validated against a Monte-Carlo area oracle** (the now-correct outputs satisfy U=I+X, D=|A|−I, X=U−I exactly and match MC within noise; NOT Clipper2). Per-pair before/after toggle over 20000 general [-40,40] quad pairs (incl. duplicate-vertex degenerate inputs) AND 8000 synthetic collinear-quad pairs (simplify firing on ~75% of them): **2 seeds fixed, ZERO regressions** — the only output-area change anywhere in 28000 pairs is the two corrected seeds. Guards `TestCollinearMidVertexSimplified` (both seeds, all four ops) and `TestSimplifyCollinearRing` (unit). The third pre-existing seed `FuzzIntersect/ff9aee9b909462b0` is a DIFFERENT class (no removable collinear vertex; cross-source collinearity between A and B) and is left as the next residual — its area is byte-identical before/after. Files: boolean.go (`appendRing` rewrite + `simplifyCollinearRing`), boolean_test.go.

   **LANDED — proper-crossing point rounds order-independently: `properIntersection` canonicalises its argument order (2026-05-22).** The last pre-existing fuzz seed, `FuzzIntersect/ff9aee9b909462b0` `A=(-10,-10),(48,48),(10,40),(-10,65) ; B=(-3,53),(3,-114),(3,99),(-36,3)`. A's bottom edge `(-10,-10)-(48,48)` lies on y=x and crosses B's vertical edge x=3 at the lattice point `(3,3)`. **Union collapsed 2516.89→147.86 and Intersect bloated 351.11→1380.49** (Difference 1268.89→628.01); Xor was correct (2165.77) because it ignores `WindOther`, which localised the bug to winding classification, not ring geometry. A throwaway sweep trace showed the crossing at `(3,3)` was dispatched TWICE in adjacent scanbeams, the second dispatch undoing the first (it swapped A's diagonal back onto the wrong ring above `(3,3)`). **Root cause:** `properIntersection` computes the crossing point by parametrising along its FIRST argument (`a.Bot + t·dir(a)`), so float rounding of `a+t·dirA` differs from `b+u·dirB` for the same geometric crossing — the two grid points landed one unit apart in Y. `doIntersections` resolves a crossing per scanbeam from the current AEL and recomputes it in the next beam after the crossing has swapped the two edges into reversed AEL order; the order-dependent rounding then placed the recomputed point just past the beam boundary, escaping the already-handled `res.P.Y <= botY` guard, so the same crossing fired again. **Fix (`properIntersection`, clip/intersect.go):** order the two segments canonically by their `(Bot, Top)` endpoints (`segCanonLess`) before computing, so the rounded result is identical regardless of caller order; the beam guard then reliably suppresses the duplicate dispatch. **Validated against a Monte-Carlo area oracle** (the corrected outputs satisfy U=I+X, D=|A|−I, X=U−I and match MC within noise; NOT Clipper2): per-pair before/after toggle over 12000 non-convex-quad pairs at coordinate half-extents 8 and 40, **ZERO regressions** (the change only re-rounds order-dependent crossings, by at most one grid unit). Full suite incl. all fuzz seed corpora green, vet + lint 0. Guards `TestCrossingSnapsOrderIndependently` (the seed, all four ops) and `TestIntersectProperCrossOrderIndependent` (unit: `Intersect(a,b).P == Intersect(b,a).P`). This was the last of the pre-existing corpus fuzz failures. Files: clip/intersect.go (`segCanonLess` + canonical ordering in `properIntersection`), boolean_test.go, clip/intersect_test.go.

   **LANDED — interiorPoint avoids horizontal-edge scanlines so touching polygons aren't nested (2026-05-22).** With all pre-existing corpus seeds fixed, a fresh degeneracy-forcing MC differential (forced shared-vertex / vertex-on-edge / collinear-edge quad pairs, coords [0,12], per-op identity + MC checks) surfaced a new gross class: two polygons that touch only along a full shared horizontal edge. Repro `A=(8,1),(12,9),(6,9),(5,6) ; B=(6,9),(12,9),(2,11),(4,7)` — A below y=9, B above, sharing the edge `[6,12]@y=9`; they only touch (I=0), so Union must be two sibling outer rings (43 = |A| 31 + |B| 12). Instead **Union and Xor collapsed to 19**: the engine emitted B's outline as a CW ring (the non-Union FrontEdge convention) and the hole→outer promotion in `assembleResult` left it nested as a hole of A. **Root cause (`interiorPoint`, boolean.go):** the hole→outer promotion only reverses a CW orphan ring to an outer when no outer encloses it, tested by sampling a point of the ring's open interior (`ringInside`). `interiorPoint` chose the scanline `Y = mean of vertex Ys`; for B that mean is exactly 9, the shared edge's Y, so the half-open crossing test skipped both horizontal edges and the widest "interior span" ran ALONG B's own top edge — returning `(7.5,9)`, a boundary point that lies on A's edge too, so `A.Contains` read it as inside and B was nested. The symmetric two-triangle case worked only because its mean Y fell off the shared edge. **Fix:** pick the scanline Y strictly between the two adjacent distinct vertex Ys with the widest gap, so it grazes no vertex and runs along no horizontal edge — a general robustness fix for all nesting decisions, not a special case. **Validated** against the degeneracy MC differential (11350 forced-degenerate pairs): the d=24 gross fail vanished with no new top-ranked fails; the remaining standouts are a separate Xor coincident-horizontal double-count class (`A=(5,9),(3,3),(12,9),(7,9) ; B=(12,9),(10,9),(2,10),(6,3)`, d≈4.6, idX violated). Full suite incl. all fuzz seed corpora green, vet + lint 0. Guards `TestTouchingAlongHorizontalEdgeNotNested` (the repro, all four ops) and `TestInteriorPointAvoidsHorizontalEdge` (unit). Files: boolean.go (`interiorPoint`), boolean_test.go.

   **LANDED — coincident horizontal hot edges interleave instead of tunnelling, so Xor keeps a shared-plateau apex (2026-05-22).** The degeneracy differential's next gross class: two polygons whose top plateaus overlap and share the right apex. Repro `A=(5,9),(3,3),(12,9),(7,9) ; B=(12,9),(10,9),(2,10),(6,3)` — A's top `(12,9)-(7,9)-(5,9)` and B's top `(12,9)-(10,9)` coincide over `[10,12]@y=9`, both interiors below. Xor over-counted **21.15 vs 16.60** (= U−I = 34.80−18.20); the `X=U−I` identity was violated → the intersection-hole was emitted self-overlapping with the union-outer. **Root cause (`branchBothHot`, clip/poly_ops.go):** SplitOverlaps fragments the two coincident plateaus into matching pieces `(10,9)-(12,9)`. At y=9 those two pieces are both FRONT hot edges of different rings (the in-progress union-outer #0 and intersection-hole #2). `dispatchIntersect` routes them to `branchBothHot`, whose **tunnel branch fires on `e1.IsFront()`** and treats the coincident pair as a point-crossing: `AddLocalMaxPoly` JOINs the hole ring #2 into the outer ring #0 and `AddLocalMinPoly` respawns a ring that `closeBound` then closes at the apex as a degenerate two-point spike `(10,9),(12,9)` — the apex triangle of the hole collapses and its area is lost (the long-standing "apex collapses to a 2-pt spike" symptom, now precisely localised via a sweep event trace). Coincident edges do not cross transversally, so the tunnel/maxima machinery is categorically wrong for them. **Fix:** in `branchBothHot`, detect coincident horizontal hot edges (identical `Bot` and `Top`) and skip the tunnel branch so they take the INTERLEAVE path — each ring keeps its own chain along the shared segment and they swap AEL ownership. With that, the raw self-touching ring carries the correct net area (16.60) and `splitSelfTouchingRings` decomposes it into the right pieces. Restricted to Xor (the other ops route coincident pairs through the opposite-side skip earlier in `dispatchIntersect`); the differential confirms U/I/D have no gross fail in this class. **Validated** against the differential: degenerate scenario gross fails 109→102 (X 47→44, idX 3→1; the d≈4.6 target gone, remaining ≤1.1 = MC noise floor), and random-small (5803 pairs) + random-large (60714 pairs, identity checks) BOTH zero — no regressions. Full suite incl. all fuzz seed corpora green, vet + lint 0. Guard `TestXorCoincidentPlateauKeepsApex` (all four ops). Files: clip/poly_ops.go (`branchBothHot`), boolean_test.go. The standing differential harness is `tools/differential` (run `go run ./tools/differential`).

   **LANDED — same-side maximum splices a terminal ring into a continuing one as a self-touching detour, so Xor keeps a vertex-on-edge apex corner (2026-05-22).** The note above mis-labelled the degeneracy scenario's residual ≤1.1 as MC noise; it was a real `idX` violation (a noise-free identity, not the MC oracle). Repro `A=(1,1),(12,8),(7,6),(5,10) ; B=(9.5,7),(8,7),(3,7),(5,2)` — B's collinear-horizontal top `(3,7)-(8,7)-(9.5,7)` meets its right edge at the plateau apex `(9.5,7)`, which lies EXACTLY on A's edge `(12,8)-(7,6)` (slope 0.4 through it; SplitTJunctions splits A's edge at `(9.5,7)`). Xor dropped the top-right corner of A, the triangle `(12,8)-(9.5,7)-(8.255,5.617)` of area ≈1.106 (X 15.46 vs U−I 16.57); U/I/D identities all held, isolating it to ring geometry under Xor's interleave. **Root cause (`AddLocalMaxPoly`, clip/output.go):** at A's apex `(12,8)` the terminal triangle ring (its other edge already closed at `(9.5,7)`) and the larger A ring (still building its left edge `(1,1)-(5,10)` up to `(5,10)`) arrive both-FRONT — the vertex-on-edge same-side degeneracy. The figure-8 PINCH that handles this is gated on BOTH rings being terminal; here one continues, so it fell to the relabel + `JoinOutrecPaths` path, which splices two same-front chains and folds the apex into a degenerate out-and-back spike `(8.255,5.617)→(12,8)→(8.255,5.617)` — exactly the failure mode the figure-8 was built to avoid, just outside its guard. **Fix:** when EXACTLY ONE ring continues and both maximum edges are FRONT, splice the terminal ring's loop into the continuing ring as a self-touching detour at the apex (the same cross-link), but KEEP the continuing ring's back edge and its growing tip — set `Pts` to the terminal apex node so `Pts.Next` is the preserved back-edge tip. The merged walk revisits the apex; `splitSelfTouchingRings` decomposes it. **Validated** against the differential: degenerate `idX` 1→0 (all of `idU/idD/idX` now 0 across 11350 forced-degenerate pairs — every noise-free identity holds), random-small (5803) + random-large (60714) BOTH zero; the remaining degenerate gross fails are all MC-oracle noise (d≤1.01 at ext=12, tol 0.6). Full suite incl. all fuzz seed corpora green, vet + lint 0. Guard `TestXorVertexOnEdgeApexKeepsCorner` (all four ops). Files: clip/output.go (`AddLocalMaxPoly`), boolean_test.go.
