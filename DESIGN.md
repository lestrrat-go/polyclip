...
- **Still open — general non-convex crossing-splice (why `TestUnionAllManyOverlapping` stays skipped). RE-DIAGNOSED 2026-05-20.** The diamond re-union gap was previously attributed to maxima/through-vertex coincidence at the shared y=10 peaks / y=7.5 valleys. **That hypothesis is disproven.** Differential evidence:
  ...
  **Locus.** `IntersectEdges` itself is a faithful Clipper2 port (verified line-by-line against engine.cpp:1872-2084, incl. the unconditional cross-polytype `AddLocalMinPoly`). The fault is the **front/back ring orientation** that feeds the maxima joins: `AddLocalMaxPoly` (output.go:135) is repeatedly entered with `IsFront(e1)==IsFront(e2)` and only completes via the swap-and-Pts-shift workaround added in commit d50048a. In Clipper2 that configuration is reachable *only* for open-ended edges (`SwapFrontBackSides` else error) — for closed-path Boolean ops it should never occur, so the orientation was already set wrong upstream: the suspect is the front/back assignment in `AddLocalMinPoly` for the crossing-spawned ring (`isNew=false`, `getPrevHotEdge`/`outrecIsAscending` path) and/or a `SwapOutrecs` at the earlier crossing. Next increment: make the crossing-spawned ring's orientation correct so the two fragments splice on the matching ends, removing the need for the d50048a workaround. Same theme as b3d5020 (concave-union orientation divergence).
- **Xor / Difference classification:** the differential's residual failures are dominated by identity violations (`X≠U−I`, `D≠a−I`) even at general position (Xor ~15% @ 2 crossings, Difference ~10% @ 2 crossings as of 2026-05-20), while Union/Intersect *bounds* mostly hold (U <1%, I ~0%). Xor/Difference need a classification audit independent of the crossing rework; the wind-count rewrite did not move these numbers (the simple-quad cases never hit the divergent winding paths).
