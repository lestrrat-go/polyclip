...
- **General non-convex crossing-splice — FIXED (2026-05-20); `TestUnionAllManyOverlapping` un-skipped and passing.** Two coupled premature-mutation bugs broke `getPrevHotEdge`'s search for the *enclosing* hot edge, which is what sets a crossing-spawned ring's front/back orientation:
  ...
  **Residual (still open).** The d50048a workaround in `AddLocalMaxPoly` is *not* yet dead: a probe (panic in that branch) still fires on a minority of random non-convex pairs in the differential, so some crossing-spawned rings are still mis-oriented. The workaround is kept as a recovery path. Removing it cleanly is a follow-up increment (likely the same root cause class in another event path — e.g. through-vertex/maxima handling or `SwapOutrecs` at deeper nesting).
