...
- **Front/back orientation divergence at concave-union maxima (partially addressed):** the deeper audit found that `AddLocalMaxPoly` is repeatedly hit with `IsFront(e1)==IsFront(e2)` at concave-union peaks (e.g. the overlapping-diamond crown). Clipper2 *never* produces same-front at a closed-path maximum (`succeeded_=false`); polyclip does, because its `IntersectEdges` keeps wind state via pre-swap snapshots + a `Classify` left-walk recompute rather than Clipper2's incremental `wind_cnt`/`wind_cnt2` updates, so front/back polarity drifts. polyclip papers over it with a "reverse e2's sides and join" path. That reverse was missing the `Pts = Pts.Next` head shift that Clipper2's `SwapFrontBackSides` (engine.cpp:460) does — adding it (commit "shift Pts head when reversing ring sides at join") roughly halved the differential crossing-pair failure rate (25% → 14%). But the same-front case should not arise at all; the diamond crown still mis-links (the tooth disconnect persists). **Real fix:** align polyclip's wind-count maintenance in `IntersectEdges` with Clipper2's incremental model so front/back polarity stays consistent and the same-front branch becomes unreachable for closed paths. `advanceBoundCursor` (through-vertex) coincidence is likely a related gap. This is a substantial, careful change (high regression risk to the now-passing convex cases).
