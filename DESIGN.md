...
**Out of scope for this increment / remaining caveats.** Degenerate positions (shared vertices, vertex-on-edge T-junctions, collinear overlaps that survive preprocess) are a separate hardening task; the differential harness should be re-run with a *generic-position* filter to confirm the crossing-count failure rates collapse, then separately with degeneracies allowed to size the remaining work.
  ...
  **RECOMMENDED NEXT increment: the sweep's classification of a different-source fully-coincident horizontal pair (the partial-collinear-overlap residual).** Trace a minimal repro (cross=0 Difference `A=(3,4),(6,6),(7,8),(2,8) ; B=(3,0),(6,4),(6,8),(1,8)`, or the Union/Xor `A=(1,2),(7,1),(8,3),(3,3) ; B=(0,2),(4,3),(8,3),(2,6)` sharing collinear horizontal y=3) through `doHorizontal` / the horizontal-AEL winding path; check whether the two coincident horizontals (one per source) get the right `WindSelf`/`WindOther` so the per-op contributing test keeps the correct boundary. Validate against the [0,8] simple-quad MC differential (NOT Clipper2, which drops these slivers at small scale — see prior note) with the off-grid-perturbation check to confirm the class is closed. The d50048a `AddLocalMaxPoly` workaround is unrelated to this class (these failures are not all Xor and several have `cross=0`) and stays until the structural OutRec-ownership track lands.
