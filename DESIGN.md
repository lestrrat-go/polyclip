...
Status (2026-05-20): **largely complete.** Implemented on branch `refactor-dohorizontal`. All six stages below landed; the full committed `clip` + `polyclip` suite is green and the four previously-skipped tests (`TestUnionOverlappingSquaresVertexInsideOther` → area 184, and `TestIntersect/Difference/XorOverlappingAxisAligned`) now pass. Horizontals are first-class AEL edges; the §11.7 synth-intersect machinery is deleted.

**Known follow-up (tracked by `TestUnionCoincidentHorizConfluence`, skipped):** a multi-edge confluence — one polygon's local-maximum apex coinciding with the other's boundary vertex, combined with a coincident bottom horizontal — is mis-handled: `closeBound` does not process the edges *between* a maxima pair the way Clipper2's `DoMaxima` (engine.cpp:2729) does, so hot/contributing status isn't transferred across the confluence and the larger ring is dropped. Fix path: port `DoMaxima`'s between-maxima `IntersectEdges` loop into `clip/sweep.go:closeBound`. The pre-rework engine handled this case via the (now-deleted) synth-intersect mechanism.
