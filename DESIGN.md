...
- **Still open — maxima / through-vertex coincidence (why `TestUnionAllManyOverlapping` stays skipped):** the diamond re-union gap is NOT the wind drift (now fixed) but the *separate* coincidence handling at shared local **maxima** (the y=10 peaks) and through-vertices (y=7.5 valleys). `AddLocalMaxPoly` / `advanceBoundCursor` still mis-link a peak ring when two bounds meet exactly on a vertex scanline. This needs the symmetric maxima/through-vertex handling analogous to the `handleLocalMin` bubble. UnionAll (535) vs cumulative (489) still diverge by reduction order, both short of 550.
