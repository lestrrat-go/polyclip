...
- **General non-convex crossing-splice — FIXED (2026-05-20); `TestUnionAllManyOverlapping` un-skipped and passing.** Two coupled premature-mutation bugs broke `getPrevHotEdge`'s search for the *enclosing* hot edge, which is what sets a crossing-spawned ring's front/back orientation:
  ...
  **Next track (C): degenerate-position crossing mis-orientation.** Root-cause why a shared-vertex / vertex-on-edge crossing is either not dispatched (Intersect→0 repro) or produces a mis-oriented ring (the same-side maximum the workaround patches). Likely in `reconcileSharedVertexCrossings`'s detection/ordering or `AddLocalMinPoly`'s `getPrevHotEdge` at a coincident vertex; compare against Clipper2's at-vertex sequencing (`DoMaxima`/`IntersectEdges` at equal points). Fix → re-run the clean-vs-degen differential → expect degen wrong-rate to drop and Xor firings → 0 → then delete the d50048a workaround with new degenerate regression guards. Separately, the **orientation invariant** (`runBooleanOp` assumes CCW-outer/CW-hole but never enforces it via `validate.go`) is a small boundary-enforcement increment worth doing on its own.
