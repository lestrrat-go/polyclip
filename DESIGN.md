...
#### 12.10.7 Lessons from the integration that the protocol must capture
...
**The gap.** The original sweep scheduled crossings *incrementally* (`maybeScheduleIntersect` ran only when two edges became AEL-adjacent through a specific event). When an adjacency formed through an AEL rearrangement that did not trigger a fresh pairwise check, the crossing was silently lost — broadly wrong for general-position sloped crossings (~56% wrong at 2 crossings, vs the Monte-Carlo oracle).

**Fix — per-scanbeam recompute (Clipper2 `DoIntersections`).** For each scanbeam `(botY, topY]` recompute *all* crossings from the settled AEL and process bottom-up: enumerate edge pairs whose `Intersect` is a `ProperCross` in the beam (every AEL edge spans the whole beam), sort by `(pt.Y, pt.X)`, dispatch via `IntersectEdges`; if rounding leaves a node's edges non-adjacent, advance to the next adjacent node first (Clipper2 `ProcessIntersectList`). O(n²)/beam (correctness-first; the merge-sort inversion counter is a later optimisation). Integrated in `run()` before the Top/LocalMin handlers; the incremental scheduler and `EventIntersection` are removed. Result: general-position crossings 0% wrong (convex), dramatically improved non-convex.

**Incremental wind-count model.** `Classify` transcribes Clipper2 `SetWindCountForClosedPathEdge` (NonZero): `WindSelf` is the higher winding of the two regions touching the edge; `WindOther` is the running sum of the other source's `WindDx` to the left. `IntersectEdges` updates both counts *in place* at the crossing and keys the §12.5 dispatch on the post-update `abs(WindSelf)` (no left-walk recompute); `isContributing` = `IsContributingClosed`. Two deliberate divergences from Clipper2:

- The AEL position swap is deferred to the *end* of `IntersectEdges` (Clipper2 runs the dispatch in pre-crossing order and swaps afterwards). `AddLocalMinPoly` resolves left/right from current AEL positions and walks `getPrevHotEdge(left)`, so the enclosing-hot-edge orientation is argument-order-independent.
- `branchNeitherHot` compares **signed** `WindOther` where Clipper2-NonZero uses **abs**. This is a behavioral no-op under polyclip's input contract (`validate.go` requires CCW outer / CW hole, so a valid input's other-source winding to the left is always 0 or +1). Not changed, to avoid robustness code for input that cannot occur.

**Validation methodology — MC oracle, NOT Clipper2.** Correctness is measured against a Monte-Carlo area oracle (checking the identities U=I+X, D=|A|−I, X=U−I, which are noise-free, plus absolute area within MC noise). Clipper2 is **not** a usable reference for degenerate small-integer inputs: at native scale it rounds fractional crossings to the integer grid and is itself wrong on all four ops (pre-scaling its input by 1e6 confirms the MC values). On these degenerate inputs polyclip's fine fixed-point grid is *more* accurate than Clipper2. The standing differential harness is `tools/differential` (`go run ./tools/differential`).

**Degenerate-position handling.** General position is correct (0% wrong); the residual work is degeneracies (shared vertex, vertex-on-edge, collinear/coincident edges) — every such failure becomes correct under off-grid perturbation, confirming it is a snap/degeneracy effect, not a structural sweep bug. Landed mechanisms:

- **Input simplification (`boolean.go`, `appendRing`).** `simplifyCollinearRing` drops collinear-through input vertices (`Orient2D(prev,cur,next)==0`) — an exact no-op for a simple polygon that removes spurious bound turns (a wrap-around dedup runs first so a repeated vertex can't be misread as collinear). Orientation is normalized: a ring is reversed when its signed area disagrees with the wanted winding (CCW outer / CW hole), so the sweep's `WindDx`-from-direction assumption holds for any input winding.
- **Preprocess invariants.** `SplitTJunctions` establishes "no vertex lies in the open interior of any edge" (sibling of `SplitOverlaps`' "no partial collinear overlaps"); split points are existing grid vertices, so area-preserving. `collinearOverlap` takes overlap endpoints directly from the input endpoints (never re-projects via `xAtY`) — re-projection rounded off-line and manufactured a non-canonical sliver that infinite-looped `SplitOverlaps`. `properIntersection` orders its two segments canonically (`segCanonLess`) so a crossing point rounds identically regardless of caller order (an order-dependent rounding dispatched the same crossing twice in adjacent beams).
- **Shared-vertex / through-vertex crossings.** `handleLocalMin` inserts the right bound adjacent and bubbles it into order, dispatching `IntersectEdges` at the local-min (Clipper2 `InsertLocalMinimaIntoAEL`). `reconcileSharedVertexCrossings` dispatches at-vertex crossings (a `Touch` on the beam boundary, invisible to `doIntersections`) by detecting adjacent edges with equal `CurrX` now out of slope order; called after the Tops phase *and* again after the horizontal flush. `handoffMaxThroughVertex` hands a hot maximum edge's ring onto a **cold** through-edge that continues strictly above the shared vertex (decided from the bound's apex via `boundContinuesAbove`, not the timing-dependent cursor segment); it defers to a genuine same-source maxima partner (a real maximum is closed by the maxima/`resolveBetweenMaxima` path).
- **Maxima pairing.** `isMaximaPartner` requires same source (Clipper2 `GetMaximaPair` pairs by apex identity). `scanMaximaPartner` also accepts a continuing-above horizontal between-edge spanning the apex column (a through-edge that turned onto a horizontal). `plateauPartnerPending` defers a plateau maximum only when its coupled horizontal partner itself tops out at the apex — a *continuing* horizontal never closes the deferred edge, leaving it hot and blocking a higher maximum's pairing.
- **Scanline phasing.** Local-max horizontal plateaus are flushed (`flushPendingHoriz`) after the Tops phase and *before* the local-minima loop (mirrors Clipper2 `ExecuteInternal`). Otherwise a closing plateau lingers in the AEL when a coincident local min is classified, inflating its `WindOther` and dropping it. This one-line ordering fix resolved the shared-collinear-horizontal class; the suspected Clipper2 horizontal-join port was unnecessary (see deadends).
- **Ring closure / same-side maxima.** polyclip's bottom-up sweep builds interleaved rings that meet **same-side** at an apex where Clipper2's top-down sweep would close a ring on itself. `AddLocalMaxPoly` merges them as a figure-8 pinch (emit the apex on each ring, cross-link the two apex OutPts into one self-touching cycle); `assembleResult`'s `splitSelfTouchingRings` decomposes that walk into simple rings — no front/back orientation guess. When exactly one ring continues (its other edge still in the AEL), the terminal ring's loop is spliced in as a self-touching detour while the continuing ring keeps its back edge and growing tip. `closeBound` Case B emits its own apex when `maxPt` differs from both chain ends (a cross-source ring whose two hot edges terminate at different points joined by a horizontal top).
- **Coincident horizontals.** A coincident different-source horizontal pair does not cross transversally. `dispatchIntersect` returns nil for an **opposite-interior** pair (read off `Segment.Reversed`) — a doubled/cancelling boundary — for non-Xor ops; the `clip/horzjoin.go` port (`processHorzJoins`) reconnects the skip-separated runs. Same-interior pairs fall through to normal dispatch. The skip is suppressed at a boundary **EXIT** (one bound continues past the overlap): `continuesCollinearHorizontal` (continuation is a collinear horizontal), `respawnHandoffAtOverlap` (continuation leaves non-horizontally; the ending bound is hot, the continuing one cold), and a `IsBoundLast` requirement (both-continue is two live boundaries, not a cancellation). For Xor, coincident horizontal hot edges **interleave** instead of tunnelling (the tunnel branch fires on `IsFront` and would collapse a shared plateau apex to a 2-point spike); Xor is excluded from the horz-join pass (an unrestricted `processHorzJoins` infinite-loops on some Xor inputs).
- **Postprocess nesting (`assembleResult`, non-polytree).** polyclip computes hole nesting in postprocess, not via a polytree. It builds a single containment forest over **all** output rings (any orientation) and classifies each by depth parity: a ring at even depth is a filled region (an ExPolygon outer), odd depth is a hole of its parent (which, being depth−1, is always an even-depth filled ring). The sweep's own CCW/CW orientation is locally meaningful but does **not** encode nesting depth, so it is used only to normalize the final winding (filled→CCW, hole→CW), never to classify — an island that sits inside a hole is CCW yet at depth 2, so it must become its own top-level piece, not a hole. Computing depth among outer rings only mis-handled that island (it saw depth 1 and demoted it to a hole, dropping the real hole). Containment is sampled at a *genuine interior point* of the inner ring (`interiorPoint`: midpoint of the widest interior span on a scanline that grazes no vertex and runs along no horizontal edge), not a vertex centroid — a centroid on a shared/collinear edge tests boundary-inclusive `Contains` as inside and falsely nests touching rings; conversely a hole all of whose vertices lie on the outer boundary is not falsely promoted. Only a strictly **larger** ring can be a container, except that two equal-area **coincident** rings (the same boundary emitted once CCW and once CW — as Difference/Xor of identical inputs do) break the tie by orientation so they nest as outer+hole and cancel to zero area.
- **Robustness guard.** `traceRing` (`bounds.go`) returns `ErrOpenRing` when its walk reaches a visited non-start segment — a self-touching input would otherwise follow a sub-cycle that never closes and grow without bound (an OOM, not just a hang).

**Refuted approaches / deadends** (tried, measured, reverted — do not retry):

- **Local discriminators on the dispatch-skip and the same-side `AddLocalMaxPoly`.** No single local predicate (both-continue, hotness, `WindOther`, winding-derived parity) can separate touch-vs-overlap or merge-vs-separate at the firing — different coincident crossings *within one op* need opposite decisions; the choice is set by global ring topology. (The opposite-interior `Reversed` discriminator and the figure-8 + AEL-derived recovery later resolved the specific classes that *do* have a structural signal.)
- **`WindDx`-derived parity in `AddLocalMinPoly`** (replacing the `outrecIsAscending` proxy): `outrecIsAscending` and `WindDx<0` do **not** coincide in general — the `FrontEdge` proxy correctly differs for legitimately hole-oriented / cross-source-merged rings. Badly regresses.
- **AEL-ordered maxima sweep** (process maxima leftmost-first instead of event-pop order): only shuffles which inputs land in the same-side recovery; not a root-cause fix.
- **Don't-split (whole-horizontal) model** (drop `SplitOverlaps`' horizontal splitting to match Clipper2): rejected — polyclip is built on the no-overlap invariant; reworking winding/ring-construction for whole overlapping horizontals regressed broadly. Keep-split + discriminated skip + join is far smaller and sufficient.
- **Clipper2 `splits`/`owner`/`SwapFrontBackSides` machinery is NOT needed.** It is all `using_polytree_`-gated in Clipper2; polyclip is non-polytree (postprocess recomputes ownership). The horizontal-join port was even deleted as **inert** under keep-split + skip — re-port only with a failing test that genuinely needs it.

**Status.** With the above, the differential's noise-free identities (idU/idD/idX) all hold across the random and forced-degenerate scenarios (single hole-free outer rings); remaining gross fails there are MC-oracle noise (near tolerance, shrink with more samples). **Open residual: inputs that contain holes.** The `holes` differential scenario (A = a square with a CW hole, B a quad that touches/crosses the hole) exposes a sweep-level bug class — the input hole's edges tangle with the clip polygon during winding classification, so the *sweep itself* emits rings whose net area is wrong (the postprocess nesting fix above only corrected output-ring assembly). Difference is the worst hit. This is the next correctness gap to close.
