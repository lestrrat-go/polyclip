...
- **Postprocess nesting (`assembleResult`, non-polytree).** polyclip computes hole nesting in postprocess, not via a polytree. Nesting is decided by sampling a *genuine interior point* of the inner ring (`interiorPoint`: midpoint of the widest interior span on a scanline that grazes no vertex and runs along no horizontal edge), not a vertex centroid — a centroid on a shared/collinear edge tests boundary-inclusive `Contains` as inside and falsely nests touching rings; conversely a hole all of whose vertices lie on the outer boundary is not falsely promoted.
...
**Status.** With the above, the differential's noise-free identities (idU/idD/idX) all hold across random and forced-degenerate scenarios; remaining differential gross fails are MC-oracle noise (near tolerance, shrink with more samples). No known correctness gap remains.
