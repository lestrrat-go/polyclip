...
- **General non-convex crossing-splice — FIXED (2026-05-20); `TestUnionAllManyOverlapping` un-skipped and passing.** Two coupled premature-mutation bugs broke `getPrevHotEdge`'s search for the *enclosing* hot edge, which is what sets a crossing-spawned ring's front/back orientation:
  ...
  **Track C — remaining (next increment): shared local-MAX confluence.** The residual single-shared-vertex failures (Union/Intersect still ~8%) are dominated by a *different* class: two sources reaching their local **maximum** at the SAME vertex — four edges (two bounds per source) converging on one point, often with a crossing just below. Repro `A=(9,2),(10,4),(8,6),(8,4) ∪ B=(7,0),(8,3),(9,4),(8,6)` (shared max (8,6)): Union 1.333 vs truth ~5.71 — everything above the lower crossing is dropped. The trace shows two `closeBound` calls and two already-removed tops at (8,6); the maxima-pairing path (`closeBound` / `maximaPartner` / `resolveBetweenMaxima`) does not correctly pair four edges from two sources at a shared apex, and the cross-source crossing below joins the rings prematurely. This is structural maxima-path work, NOT a reconcile tweak — a separate increment. Compare Clipper2 `DoMaxima`/`GetMaximaPair` at coincident apexes. Then: re-run the clean-vs-degen differential → drive Union/Intersect degen rate down and Xor firings → 0 → delete the d50048a workaround. Separately, the **orientation invariant** (`runBooleanOp` assumes CCW-outer/CW-hole but never enforces it via `validate.go`) is a small boundary-enforcement increment worth doing on its own.
