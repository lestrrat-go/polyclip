...
- **Different-source crossing on a vertex scanline joining a complex ring:** re-unioning the overlapping diamonds (shared y=10 peaks, y=7.5 valleys) disconnects a peak ring. Isolated precisely: the crown alone unions fine (the maxima-join/`closeBound` path is correct); the corruption is triggered by the blob×d3 crossing at exactly `(12.5, 7.5)` — coincident with the valley scanline. `IntersectEdges` there hits `branchBothHot`'s `!samePolyType && op==Union` arm → `AddLocalMaxPoly`, which joins blob's mid-construction crown OutRec; the join mishandles that OutRec's front/back links (the disconnected tooth is on the far left, away from the crossing — a global OutRec-linking tangle, not a local error). This is a §12.5 decision-tree / `AddLocalMaxPoly`-join robustness bug surfaced (not caused) by the per-scanbeam recompute processing the crossing at the scanline boundary. NOT a quick fix — needs the §12.5 join logic audited for complex/multi-join OutRecs. `advanceBoundCursor` (through-vertex) coincidence is likely a related gap.
